  ...
  <div class="vip vip-manager" @click.stop="rightClickHandle($event, {})">
    ...
    <div class="search m-t-2 m-b-4">
      ...
      <div class="row" layout="row" layout-align="start center">
        <el-input class="m-r-2" v-model="form.keyword" size="small" placeholder="姓名/手机号/会员卡号" style="width:200px"></el-input>
      ...
      <div class="table">
        ...
        <div class="tbody">
          ...
          >
            ...
            <ul class="tips" v-if="item.showTips" :style="{'left': item.pointerX + 65 + 'px', 'transform': 'translate(-50%,' + item.disY * -1 + 'px)'}">
...
export default {
  ...
  methods: {
    ...
    getTipsList(type) {
      ...
      return tipsArr.filter(item => (type == 1 ? makedCardList.includes(item.id) : notMakedCardList.includes(item.id)))
    },
  ...
  watch: {
    'form.typeVal'(newVal) {
      const currentTypeInfo = this.form.typeOption.find(item => item.id == newVal) || {}
