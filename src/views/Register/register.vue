    ...
    <div class="contain" layout="row" layout-align="center center">
      ...
      <div class="register-contain" :style="{transform: 'translate(-50%, -50%) scale(' + this.scale + ')'}">
        ...
        <div class="tips" layout="row" layout-align="center center">
          <div class="close" layout="row" layout-align="start center" @click="exitHandle">
            <img :src="require('@/assets/register-login/close_sys.png')" alt />
            <span>关闭系统</span>
          </div>
        </div>
        <div class="effect" v-if="showEffectArea" @click="showEffectHandle"></div>
        <div v-if="projectName=='dev' || projectName=='105'" @click="showEffect=true">click me</div>
      </div>
    </div>

    <!-- 系统选择模态框 -->
    <el-dialog title="请选择当前需要登录的系统" :visible.sync="showEffect" :show-close="false" :close-on-click-modal="false" :close-on-press-escape="false">
      <ul class="effect">
        <li v-for="(item,i) in effectList" :key="i">
          <el-radio v-model="effectRadio" :label="i" :disabled="item.disabled">{{item.name}}</el-radio>
        </li>
      </ul>
...
export default {
  ...
  methods: {
    ...
    async submitHandle() {
      ...
      if(machine_code == '') return this.$message.warning('当前设备不支持授权')
      const params = {
