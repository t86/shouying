...
export default {
  ...
  methods: {
    async asyncDoHandle(){
      const params = {
        op_status: this.effectType == 11 || this.effectType == 21 ? 2 : (this.effectType == 12 || this.effectType == 22 ? 1 : 3),
        type_ids: this.nexDrawerInfo.cs.map(item => item.tid),
        ids: this.nexDrawerInfo.cs.map(item => item.id)
      }
      try {
        const method = this.effectType == 11 || this.effectType == 12 || this.effectType == 13 ? 'dept' :'emp'
        const api = this.effectType == 11 || this.effectType == 12 || this.effectType == 13 ? 'requestdeptOb' :'requestEmpOb'
  ...
  props: {
    ...
    effectType: {
      default: 1  // 11/21无效  12/22 有效 13/23 删除
    },
    nexDrawerInfo: {
      default: () => ({})
    }
  ...
  computed: {
    title() {
      return this.effectType == 11 || this.effectType == 21 ? "置为无效" : (this.effectType == 12 || this.effectType == 22) ? "置为有效" : "批量删除";
    },
