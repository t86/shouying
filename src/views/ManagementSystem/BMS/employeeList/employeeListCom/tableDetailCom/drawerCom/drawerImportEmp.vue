            ...
            <div class="table">
              ...
              >
                <div class="tr" layout="row" layout-align="space-between center" v-for="(item, i) in ImportData" :key="item.id">
                  ...
                  <div class="td">{{item.codes}}</div>
                  <div class="td">{{item.depts}}</div>
...
export default {
  ...
  methods: {
    ...
    async downloadTemplate() {
      try {
        const res = await this.$api.BMS.emp.requestEmpTpl()
  ...
  computed: {
    ...
    http() {
      return (process.env.NODE_ENV === 'development' ? projectConfig[projectName]['development'] : projectConfig[projectName]['product']) + '/emp/imp_chk'
    },
