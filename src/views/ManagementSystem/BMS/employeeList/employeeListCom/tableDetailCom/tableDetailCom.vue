  ...
  <div class="table-detail">
    <div class="group">
      ...
      <div class="top" layout="row" layout-align="start center">
        <icon-button
          @click.native="addOrUpdateHandle(11)"
          text="新增"
          img="btn_add.png"
          colors="#383943"
        ></icon-button>
        <icon-button
          @click.native="addOrUpdateHandle(12)"
          text="编辑"
          img="btn_edit.png"
          colors="#383943"
        ></icon-button>
        <icon-button
          @click.native="setEffectOrNotEffect(11)"
          text="置为无效"
          img="btn_in_vain.png"
          colors="#383943"
        ></icon-button>
        <icon-button
          @click.native="setEffectOrNotEffect(12)"
          text="置为有效"
          img="btn_effective.png"
          colors="#383943"
        ></icon-button>
        <icon-button
          @click.native="setEffectOrNotEffect(13)"
          text="批量删除"
          img="btn_delete.png"
          colors="#6B2830"
        ></icon-button>
    ...
    <div class="employee m-t-6">
      ...
      <div class="top" layout="row" layout-align="start center">
        <icon-button
          @click.native="addOrUpdateHandle(21)"
          text="新增"
          img="btn_add.png"
          colors="#383943"
        ></icon-button>
        <icon-button
          @click.native="addOrUpdateHandle(22)"
          text="编辑"
          img="btn_edit.png"
          colors="#383943"
        ></icon-button>
        <icon-button
          @click.native="setEffectOrNotEffect(21)"
          text="置为无效"
          img="btn_in_vain.png"
          colors="#383943"
        ></icon-button>
        <icon-button
          @click.native="setEffectOrNotEffect(22)"
          text="置为有效"
          img="btn_effective.png"
          colors="#383943"
        ></icon-button>
        <icon-button
          @click.native="setEffectOrNotEffect(23)"
          text="批量删除"
          img="btn_delete.png"
          colors="#6B2830"
        ></icon-button>
        <characters-button
          @click.native="anotherEffectHandle(1)"
          colors="#383943"
          wz="批量修改部门"
        ></characters-button>
        <characters-button
        ...
        ></characters-button>
        <characters-button
          @click.native="anotherEffectHandle(3)"
          colors="#383943"
          wz="批量导入"
        ></characters-button>
        <characters-button
          @click.native="anotherEffectHandle(4)"
          colors="#383943"
          wz="批量重置密码"
        ></characters-button>
        <characters-button
            ...
            >
              ...
              <div class="td">
                <span
                  ...
                  @click="showOrHideBindHandle(item)"
                  >{{
                    item.bs == "未绑定"
                      ? "绑定员工"
                      : item.bs == "已绑定"
                ...
                <span
                  class="primary-link cursor"
                  @click="makeCardHandle(item)"
                  >{{ item.wc == 1 ? "清卡" : "制卡" }}</span
                >
                <span
                  class="primary-link cursor"
                  v-if="item.wc == 1"
                  @click="destoryCard(item)"
                  >作废卡</span
                >
              </div>
            </div>
...
export default {
  ...
  methods: {
        ...
        case 23: // 类似创建
          if (type == 12 || type == 22 || type == 23) {
            ...
            if (checkedList.length != 1)
              return this.$message.warning("请选择一个进行编辑");
      ...
      switch (type) {
        ...
        case 13: // 删除部门
          ...
          if (checkedList.length <= 0)
            return this.$message.warning("请选择后进行操作");
          params = {
    ...
    anotherEffectHandle(type) {
      // type: 1：批量修改部门  2：批量修改直属上级  3：批量导入  4：批量重置密码 5：批量修改岗位
      if (type == 3) {
        this.showImportDrawer = true;
      } else {
        const checkedList = this.employeeTableData.filter(
          (item) => item.checked
        );
        if (checkedList.length <= 0)
          return this.$message.warning("请选择需要操作的员工");
    ...
    changeCheckboxHandle(type) {
      switch (type) {
        case "groupAll":
          ...
          break;
        case "groupItem":
          this.groupCheckAll = this.groupTableData.every(
            (item) => item.checked
          );
          break;
        case "empAll":
          ...
          break;
        case "empItem":
          this.empCheckAll = this.employeeTableData.every(
            (item) => item.checked
          );
          break;
      }
    ...
    showOrHideBindHandle(itemInfo = {}) {
      if (itemInfo && itemInfo.bs == "解绑中" && !this.showBindEmpDrawer)
  ...
  computed: {
    isIndeterminate() {
      return (
        !this.groupCheckAll && this.groupTableData.some((item) => item.checked)
      );
    },
    empIsIndeterminate() {
      return (
        !this.empCheckAll && this.employeeTableData.some((item) => item.checked)
      );
    },
  },
