<template>
  <div class="one-table">
    <div class="top" layout="row" layout-align="start center">
      <icon-button @click.native="addOrUpdateHandle(1)" text="新增岗位" img="btn_add.png" colors="#383943"></icon-button>
      <icon-button @click.native="addOrUpdateHandle(2)" text="编辑岗位" img="btn_edit.png" colors="#383943"></icon-button>
      ...
      <characters-button @click.native="setConfigHandle(1)" colors="#383943" wz='登录权限'></characters-button>
      <characters-button @click.native="setConfigHandle(2)" colors="#383943" wz='可点区域'></characters-button>
      <characters-button @click.native="setConfigHandle(3)" colors="#383943" wz='可点商品'></characters-button>
      <characters-button @click.native="setConfigHandle(4)" colors="#383943" wz='优惠'></characters-button>
      <characters-button @click.native="setConfigHandle(5)" colors="#383943" wz='优惠2'></characters-button>
      <characters-button @click.native="setConfigHandle(6)" colors="#383943" wz='特饮小费关联商品'></characters-button>
      <characters-button @click.native="setConfigHandle(7)" colors="#383943" wz='erp系统权限'></characters-button>
      <characters-button @click.native="setConfigHandle(8)" colors="#383943" wz='配置权限'></characters-button>
      <characters-button @click.native="setConfigHandle(9)" colors="#383943" wz='查单权限'></characters-button>
    </div>
    <div class="table-content">
      <div class="table">
        ...
        <!-- 新增或编辑 -->
        <drawerAddOrUpdateStation v-model="showAddOrUpdateDrawer" :currentInfo="currentInfo" :type="type" @getTableData="getTableData" />
        <!-- 置为有效无效删除冲突 -->
        ...
        <!-- 登录权限 -->
        <drawerLoginConfig v-model="showLoginDrawer" :checkedList="checkedList" @getTableData="getTableData" />
        <!-- 可点区域 -->
        <drawerCanOrderArea v-model="showCanOrderAreaDrawer" :checkedList="checkedList" @getTableData="getTableData" />
        <!-- 可点商品 -->
        <drawerCanOrderPrd v-model="showCanOrderPrdDrawer" :checkedList="checkedList" @getTableData="getTableData" />
        <!-- 优惠/优惠2 -->
        <drawerCanYHPrd v-model="showCanYHPrdDrawer" :type="yhType" :checkedList="checkedList" @getTableData="getTableData" />
        <!-- 特饮小费 -->
        <drawerTYPrd v-model="showTYPrdDrawer" :checkedList="checkedList" @getTableData="getTableData" />
        <!-- erp管理 -->
        <drawerErpConfig v-model="showErpDrawer" :checkedList="checkedList" @getTableData="getTableData" />
        <!-- 配置权限 -->
        <drawerSetConfig v-model="showSetConfigDrawer" :checkedList="checkedList" @getTableData="getTableData" />
        <!-- 查单权限 -->
        <drawerLookOrder v-model="showLookOrderDrawer" :checkedList="checkedList" @getTableData="getTableData" />
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async setEffectOrNotEffect(type){
      ...
      try {
        const api = type == 1 ? 'requestStationDisable' : (type == 2 ? 'requestStationEnable' : 'requestStationDel')
    ...
    setConfigHandle(type) {
      ...
      switch (type) {
        ...
        case 4: // 优惠
        case 5: // 优惠2
          if(checkedList.length != 1) return this.$message.warning('请选择一个岗位进行配置')
  ...
  components: {
    ...
    drawerCanOrderArea: () => import('./stationConfigCom/drawerCanOrderArea.vue'),
