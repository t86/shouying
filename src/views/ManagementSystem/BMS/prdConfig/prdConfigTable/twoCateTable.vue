...
export default {
  ...
  methods: {
    async getTableData(isScrollToBottom = false) {
      ...
      try {
        const res = await this.$api.BMS.tcate.requesttcatelist(params);
        if (res.code == 1) {
    ...
    async setEffectOrNotEffect(type){
      ...
      try {
        const api = type == 1 ? 'requesttcatedisable' : (type == 2 ? 'requesttcateenable' : 'requesttcatedel')
    ...
    async dragend(item) {
      if (this.oldItem != this.newItem) {
        ...
        try {
          const res = await this.$api.BMS.tcate.requesttcatemove_dsp(params)
          if(res.code == 1) {
