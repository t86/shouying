  ...
  <div class="menu-list">
    <div class="nav">
      ...
      <div class="nav-content" :style="{ 'margin-left': navWidth + 10 + 'px' }">
        <oneCateTable v-if="menuIndex == 0" ref="oneCateTable" @getMenuList="getMenuList" />
        <twoCateTable v-if="menuIndex == 1" ref="twoCateTable" :menuList="menuList" @getMenuList="getMenuList"/>
        <prdTable v-if="menuIndex == 2" ref="prdTable" :menuList="menuList" @getMenuList="getMenuList" />
        <!-- 新增或编辑单品 -->
        <addOrUpdatePrd v-model="showDrawer" :currentInfo="{id: prdId}" type="2" :search="true" @initSearchValHandle="initSearchValHandle" />
        <!-- 新增或编辑套餐 -->
        <addOrUpdateGroupPrd v-model="showGroupDrawer" :currentInfo="{id: prdId}" :type="2" :search="true" @initSearchValHandle="initSearchValHandle" />
      </div>
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    async getSearchMenuData(keyword) {
      ...
      try {
        ...
        if(res.code == 1) {
          ...
          this.searchMenu.prdList = res.data.filter(item => item.t == 3 || item.t == 4)
    ...
    getSearchDetail(itemInfo, type) {
      if(type == 'oneCate' || type == 'twoCate') {
        ...
        this.$router.push('/BMS/prdConfig?menuIndex=' + menuIndex + '&menuId=' + menuId + '&search=true')
        this.searchVal = "";
