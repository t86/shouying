<template>
    <div class="emp-list">
        ...
        <div class="emp-list-controller">
            ...
            <characters-button @click.native="DownloadBatch" colors="#383943" wz='批量导入'></characters-button>
            <characters-button @click.native="isShowResetPwd" colors="#383943" wz='批量重置密码'></characters-button>
        </div>
            ...
            <div style="height:100%" class="meisfgr">
                <table class="zhuixiaoi" style="min-width: 970px;height:100%">
                    ...
                    <tr v-for="(item,i) in tableDatas" :key="item.id" class="drag-list"
                        ...
                        <td style="width:60px" :style="{'color':item.s=='无效'?'#595959':'',}">
                            <ellipsis-tooltip :text="item.np || '-----'"></ellipsis-tooltip>
                        </td>
                        <td :style="{'color':item.s=='无效'?'#595959':'',}">
                            <ellipsis-tooltip :text="item.c || '-----'"></ellipsis-tooltip>
                        </td>
                        <td style="width:150px" :style="{'color':item.s=='无效'?'#595959':'',}">
                          <el-link :underline="false" type="primary" @click.native="showOrHideBindHandle(item)">{{item.bs == '未绑定'?'绑定员工': item.bs=='已绑定' ? '解除绑定' : '解绑中'}}</el-link>
                          <el-link class="m-l-1" :underline="false" type="primary" @click.native="makeCardHandle(item)">{{item.wc == 1 ? '清卡': '制卡'}}</el-link>
                          <el-link class="m-l-1" :underline="false" type="primary" v-if="item.wc==1" @click.native="destoryCard(item)">作废卡</el-link>
                        </td>
                    </tr>
        ...
        <div v-show="dialogBindVisible" class="dialogFormVisible">
            <el-dialog :title="currentBindPersonInfo.bs=='未绑定' ? '绑定员工' : '解除绑定'" :show-close="false" width="36%"
                       :visible.sync="dialogBindVisible" :close-on-click-modal="false">
                <div style="width:100%;height:100%" layout="column" layout-align="center center" v-if="currentBindPersonInfo.bs=='未绑定'">
                  <img width="300" :src="QRBindImgSrc" />
                  <h3 style="line-height:60px;text-align:center">请‘{{currentBindPersonInfo.n}}’用微信扫描二维码进行绑定</h3>
                  <p style="text-align:center;color:red">备注：二维码在15分钟内有效</p>
                </div>
                <h3 v-else style="line-height:60px">是否确认解除绑定？</h3>
                <span slot="footer" class="dialog-footer">
                    <el-button @click.stop="showOrHideBindHandle">{{currentBindPersonInfo.bs=='未绑定'?'关闭':'取消'}}</el-button>
        ...
        <div v-show="dialogFormVisible" class="dialogFormVisible">
            <el-dialog :title="title?'新增员工':'编辑员工'" @close="dialogFormVisible = false"
                ...
                <el-form label-position="left">
                    ...
                    <div class="compatibility">
                        ...
                        <el-select v-model="stationId" placeholder="请选择岗位" class="controlling"
                                   ref="stationRef" @change="limitingRule(2)">
                    ...
                    <div class="compatibility">
                        ...
                        <el-input :maxlength="11" oninput="this.value = this.value.replace(/[^0-9]/g, '');"
                                  v-model="phoneNum" placeholder="请输入员工电话" @blur="limitingRule(3)"
                                  ref="phoneNumRef" class="controlling"/>
                    ...
                    <div layout="row" style="width:100vw;height:100vw;background:rgba(0,0,0,.3);position:fixed;left:0;top:0;z-index:999" @keyup.esc="showMakeCardModal=false" v-if="showReloadEmpCodeModal">
                      <div style="padding:30px 20px;width:400px;background:#fefefe;position:absolute;top:40vh;left:50%;transform:translate(-50%,-50%);border-radius:10px">
                        ...
                        <div layout="row" layout-align="center center">
                          ...
                          <el-button type="primary" @click="getEmpGencode">确认</el-button>
                        </div>
                      </div>
                    </div>
                </el-form>
        ...
        <div v-show="editDept" class="dialogFormVisible editDept">
            <el-dialog title="批量修改部门" @close="editDept = false"
                       :visible.sync="editDept" :close-on-click-modal="false">
                <el-form label-position="left">
                    <div class="compatibility">
                        <div class="mandatory">
                            <span class="reqfieldsd">*</span>部门：
                        </div>
                        <el-cascader
                                style="width:350px"
                                placeholder="请选择部门"
        ...
        <div v-show="editUpper" class="dialogFormVisible editUpper">
            <el-dialog title="批量修改直属上级" @close="editUpper = false"
                       :visible.sync="editUpper" :close-on-click-modal="false">
                <el-form label-position="left">
                    <div class="compatibility">
                        <div class="mandatory">
                            <span class="reqfieldsd">*</span>员工直属上级：
        ...
        <div v-show="dialogiblecl" class="dialogFormVisblee">
            <el-dialog title="批量重置密码" :visible.sync="dialogiblecl" @close="dialogiblecl = false"
                       :close-on-click-modal="false">
        ...
        <div class="tolead" v-show="batchImport">
            <el-dialog title="批量导入" :visible.sync="batchImport" @close="batchImport = false"
                       :close-on-click-modal="false">
                <div style="margin-top: 20px;">
                    ...
                    <div style="color: #7298FF; text-align: center;margin: 5px 0;" @click="DownloadTemplate">点击下载模板
                    </div>
                    <div style="color: #FF2F4B; text-align: center;">！导入后将新增员工</div>
                </div>
                <span slot="footer" class="dialog-footer">
        ...
        <div class="feedback" v-show="feedback">
            <el-dialog title="导入员工" :visible.sync="feedback" @close="resumes" :close-on-click-modal="false">
                <div class="feedback-check">
                    共导入{{ImportData.length}}条数据，成功处理{{dataAnalysis}}条，可选择成功处理的正常数据导入或根据反馈信息修改后重新导入<br>
                    *号标识代表必填项，没有则为选填<br>
                    此序号对应文档中序号
                </div>
                ...
                <div class="juzhong">
                    <table class="table">
                        ...
                        <tr>
                            <th>
                                <el-checkbox :indeterminate="isIndeterminatedata" v-model="checkAlldata"
                                             @change="handleCheckAllChangedata">序号 1,2
                                </el-checkbox>
                            </th>
                            <th>员工名称</th>
                            <th>员工工号</th>
                ...
                'border-bottom':ImportData.length<1?'0px':''}">
                        <tr v-for="(item, i) in ImportData" :key="item.id" class="drag-lists">
                            <td>
                                <el-checkbox-group v-model="checkedCitiesdata" @change="handleCheckedCitiesChangedata">
                            ...
                            <td style="padding: 11px 0;box-sizing: border-box">
                                <div v-show="!item.err_msgs" style="color: #7298FF;line-height: 18px;">正常数据<span
                                        v-for="(result, si) in item.err_jg" :key="si">{{result}}</span>
                                </div>
                                <div v-show="item.err_msgs" style="color: #FF2F4B;line-height: 18px;"
                                     v-for="(result, si) in item.err_msgs" :key="si">{{result}}
                                </div>
                            </td>
                        </tr>
                        </tbody>
...
export default {
  ...
  data(){
    return {
      ...
      http: (process.env.NODE_ENV === 'development' ? projectConfig[projectName]['development'] : projectConfig[projectName]['product']) + '/emp/imp_chk',//路径
  ...
  methods: {
    ...
    saveDept(){
      ...
      this.$api.BMS.emp.requestEmpUpdDept({
    ...
    saveUpper(){
      ...
      this.$api.BMS.emp.requestEmpBatchUpdUpper({
        emp_ids: this.multipleSelection,
    ...
    //批量重置密码
    batchResetPwd(){
      this.$api.BMS.emp.requestEmpBatchResetPwd({
...
//          this.$parent.demand()
    ...
    construction(){
      ...
      if (this.mistake != '') {
        ...
        this.$api.BMS.emp.requestEmpNew({
    ...
    compileprinter(){
      if (this.multipleSelection.length == 1) {
        ...
        this.stationRefMistake = '';
        this.mistakephoneNum = '';
        this.$api.BMS.emp.requestEmpGet({id: this.multipleSelection[0]}).then(res => {
          if (!res.data) return;
          ...
          this.secondEmpId = res.data.emp.clone_emp_id ? res.data.emp.clone_emp_id + "" : ""
    ...
    noneffective(){
      if (this.multipleSelection.length > 0) {
        this.$api.BMS.emp.requestEmpDisable({ids: this.multipleSelection}).then(res => {
          if (res.code == 1) {
            this.demand()
            this.weik()
    ...
    effective(){
      if (this.multipleSelection.length > 0) {
        this.$api.BMS.emp.requestEmpEnable({ids: this.multipleSelection}).then(res => {
          if (res.code == 1) {
            this.demand()
            this.weik()
            this.dialogFible = false
    ...
    printerdel(){
      if (this.multipleSelection.length > 0) {
        this.$api.BMS.emp.requestEmpDel({ids: this.multipleSelection}).then(res => {
          if (res.code == 1) {
            this.demand()
            this.weik()
            this.dialogFible = false
    ...
    showOrHideBindHandle(itemInfo = {}){
      if(itemInfo && itemInfo.bs == '解绑中' && !this.dialogBindVisible) return this.$message.warning('解绑中，请勿操作')
...
<style lang="less">
    ...
    .emp-list .el-dialog__title, .emp-list .el-dialog__headerbtn .el-dialog__close, .emp-list .el-form-item__label {
        color: #1a1a21;
    }
    ...
    .emp-list .diagrgeesfelogFor .wentlb .mmodity th:nth-of-type(1), .emp-list .diagrgeesfelogFor .wentlb .drag-list td:nth-of-type(1) {
        width: 50px;
    }

    .emp-list .diagrgeesfelogFor .wentlb .mmodity th:nth-of-type(2), .emp-list .diagrgeesfelogFor .wentlb .drag-list td:nth-of-type(2) {
        width: 320px;
    }

    .emp-list .diagrgeesfelogFor .wentlb .mmodity th:nth-of-type(3), .emp-list .diagrgeesfelogFor .wentlb .drag-list td:nth-of-type(3) {
        width: 200px;
    }

    .emp-list .diagrgeesfelogFor .danykz .mmodity th:nth-of-type(2), .emp-list .diagrgeesfelogFor .danykz .drag-list td:nth-of-type(2) {
        width: 520px;
    }

    /* 无效颜色 */
    .emp-list .diagrgeesfelogFor .el-button--primary.is-disabled, .emp-list .diagrgeesfelogFor .el-button--primary.is-disabled:hover {
    ...
    .emp-list .drag-list td:nth-of-type(1), .emp-list .mmodity th:nth-of-type(1) {
        width: 70px;
    }

    .emp-list .mmodity th:nth-of-type(2), .emp-list .drag-list td:nth-of-type(2) {
        width: 150px;
    }

    .emp-list .drag-list td:nth-of-type(3), .emp-list .mmodity th:nth-of-type(3) {
        width: 150px;
    }

    .emp-list .drag-list td:nth-of-type(4), .emp-list .mmodity th:nth-of-type(4) {
        width: 100px;
    }

    .emp-list .drag-list td:nth-of-type(5), .emp-list .mmodity th:nth-of-type(5) {
        width: 100px;
    }

    .emp-list .drag-list td:nth-of-type(6), .emp-list .mmodity th:nth-of-type(6) {
        width: 100px;
    }

    .emp-list .drag-list td:nth-of-type(7), .emp-list .mmodity th:nth-of-type(7) {
        width: 150px;
    }

    .emp-list .drag-list td:nth-of-type(8), .emp-list .mmodity th:nth-of-type(8) {
        width: 100px;
    }

    .emp-list .drag-list td:nth-of-type(9), .emp-list .mmodity th:nth-of-type(9) {
        width: 150px;
    }
    ...
    .emp-list .Pagination .el-pagination.is-background .btn-next, .emp-list .Pagination .el-pagination.is-background .btn-prev, .emp-list .Pagination .el-pagination.is-background .el-pager li {
        background-color: rgba(0, 0, 0, 0);
    ...
    .emp-list .Pagination .el-pagination .btn-next .el-icon, .emp-list .Pagination .el-pagination .btn-prev .el-icon {
        color: #40404e;
    }

    .emp-list .Pagination .el-pagination.is-background .el-pager li:not(.disabled).active {
        color: #409EFF;
        background-color: rgba(0, 0, 0, 0);
    ...
    .emp-list .feedback .juzhong .drag-lists td:nth-of-type(1), .emp-list .feedback .juzhong thead th:nth-of-type(1) {
        width: 100px;
    }

    .emp-list .feedback .juzhong .drag-lists td:nth-of-type(2), .emp-list .feedback .juzhong thead th:nth-of-type(2) {
        width: 200px;
    }

    .emp-list .feedback .juzhong .drag-lists td:nth-of-type(3), .emp-list .feedback .juzhong thead th:nth-of-type(3) {
        width: 120px;
    }

    .emp-list .feedback .juzhong .drag-lists td:nth-of-type(4), .emp-list .feedback .juzhong thead th:nth-of-type(4) {
        width: 120px;
    }

    .emp-list .feedback .juzhong .drag-lists td:nth-of-type(5), .emp-list .feedback .juzhong thead th:nth-of-type(5) {
        width: 100px;
    }

    .emp-list .feedback .juzhong .drag-lists td:nth-of-type(6), .emp-list .feedback .juzhong thead th:nth-of-type(6) {
        width: 50px;
    }

    .emp-list .feedback .juzhong .drag-lists td:nth-of-type(7), .emp-list .feedback .juzhong thead th:nth-of-type(7) {
        width: 100px;
    }

    .emp-list .feedback .juzhong .drag-lists td:nth-of-type(8), .emp-list .feedback .juzhong thead th:nth-of-type(8) {
        width: 250px;
    }
