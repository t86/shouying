        ...
        <div class="zsykd" style="display: inline-block;">
            <table class="zhuixiaoi">
                ...
                <tr v-for="(item,i) in tableData" :key="item.id" class="drag-list">
                    ...
                    <td :style="{'color':item.s=='无效'?'#595959':''}">{{item.rc || '---'}}</td>
                    <td :style="{'color':item.s=='无效'?'#595959':''}">{{item.mt == 1?'单选':'多选'}}</td>
                </tr>
...
<style>
    ...
    .require_type .nodata img {
        width: 60px;
        height: 60px;
    }

    .require_type .nodata .hint {
        line-height: 36px;
    ...
    .require_type .zhuixiaoi {
        width: 1400px;
        border-collapse: collapse;
        color: #40404e;
    }

    .require_type .zhuixiaoi .mmodity tr th {
        height: 36px;
        line-height: 36px;
        text-align: left;
        font-size: 14px;
    ...
    .require_type .zhuixiaoi .limiting .drag-list td {
        height: 36px;
        line-height: 36px;
        text-align: left;
        font-size: 14px;
    ...
    .require_type .zhuixiaoi .limiting .drag-list {
        display: block;
        display: flex;
    }

    .require_type .zhuixiaoi .mmodity {
        width: 100%;
        display: block;
    }

    .require_type .zhuixiaoi .mmodity tr {
        background-color: #f5f5f5;
        display: flex;
    }

    .require_type .zhuixiaoi .limiting {
        overflow-y: auto !important;
        max-height: 540px;
        display: block;
    }

    .require_type .drag-list td:nth-of-type(1), .require_type .mmodity th:nth-of-type(1) {
        width: 100px;
    }

    .require_type .mmodity th:nth-of-type(2), .require_type .drag-list td:nth-of-type(2) {
        width: 300px;
    }

    .require_type .mmodity th:nth-of-type(3), .require_type .drag-list td:nth-of-type(3) {
        width: 200px;
    }

    .require_type .mmodity th:nth-of-type(4), .require_type .drag-list td:nth-of-type(4) {
        width: 200px;
    }

    .require_type .drag-list:nth-of-type(odd) {
        background-color: #f9f9f9;
        color: #40404e;
    }

    .require_type .drag-list:nth-of-type(even) {
        background-color: #f5f5f5;
        color: #40404e;
    }
</style>
