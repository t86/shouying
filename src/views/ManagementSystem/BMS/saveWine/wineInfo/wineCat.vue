<template>
  <div class="wine-info">
    <div class="top" layout="row" layout-align="start center">
      ...
      <characters-button @click.native="showDraweUpdate" colors="transparent" wz='编辑'></characters-button>
    ...
    <div class="table-content">
      <div class="table">
        ...
        <div class="tbody">
          <div class="tr" :class="{ 'selected': item.checked }" v-for="(item, index) in tableData" :key="index"
...
export default {
  ...
  methods: {
    ...
    showDraweUpdate() {
      if (!this.showCatDrawer && this.ids.length == 0) return this.$message.warning('请选择一个进行操作')
      if (this.ids.length > 1) return this.$message.warning('只能选择一个进行操作')
      this.status = 2;
      this.showCatDrawer = !this.showCatDrawer;
      this.currentInfo = this.tableData.find(d => d.id === this.ids[0]);
    },

    async deleteHandle() {
      if (this.ids.length == 0) {
        return this.$message.warning('请选择进行操作')
      }
      const params = {
        ids: this.ids //   []int64  请求id数组
      };
      try {
        const res = await this.$api.BMS.saveWine.reqBatchDelWineCate(params);
        if (res.code == 1) {
