<template>
  <!-- 入库明细统计 -->
  <div class="save-all-data-report">
    ...
    <div class="tab m-t-4" layout="row" layout-align="start center">
      <div class="tab-item" :class="{active: tabIndex == 1}" @click="tabIndex = 1">入库明细统计</div>
      <div class="tab-item" :class="{active: tabIndex == 2}" @click="tabIndex = 2">入库网格统计</div>
    </div>

    <!-- 入库明细统计 -->
    <saveDetail v-if="tabIndex == 1" :tableData="tableData" />
    <!-- 入库网格统计 -->
    <saveGrid v-if="tabIndex == 2" :tableData="tableData" />
...
import saveDetail from './saveAllDataReportCom/saveDetail.vue'
import saveGrid from './saveAllDataReportCom/saveGrid.vue'
export default {
  data() {
    return {
      tabIndex: 1,
  ...
  methods: {
    async getTableData() {
      ...
      try {
        const res = this.tabIndex == 1 ? await api_wine.reqSaveDetailReportList(params) : await api_wine.reqGridDetailOfSaveReportList(params);
        if (res.code == 1) {
        ...
        console.log( this.tabIndex == 1 ? '入库明细统计' : '入库网格统计' + "数据获取失败", error);
      }
    },
    ...
    async exportExcelHandle(){
      ...
      try {
        const res = this.tabIndex == 1 ? await api_wine.reqExportExcelOfSaveDetailReport(params) : await api_wine.reqExportExcelOfSaveGridDetailReport(params);
        if (!res.msg) {
