          ...
          <div class="table">
            <div class="thead">
              <div class="tr" layout="row" layout-align="space-between center">
                ...
                <div class="th">红冲实际入库数量</div>
                <div class="th">红冲后入库数量</div>
                <div class="th">红冲小计</div>
                <div class="th">实际入库小计</div>
              </div>
            </div>
            <div class="tbody">
              <div class="tr" layout="row" layout-align="space-between center" v-for="(item, i) in tableData" :key="item.id">
                ...
                <div class="td">{{item.c - (item.hcCount || 0)}}</div>
                <div class="td">{{((item.a/item.c) * (item.hcCount || 0)).toFixed(2)}}</div>
...
export default {
  ...
  methods: {
    ...
    async getAllData(){
      ...
      try {
        const res = await  this.$api.ERP.sin.requestsinget_f_hc(params)
        if(res.code == 1) {
          this.info = res.data || {}
    ...
    async onSubmit (){
      ...
      try {
        const res = await this.$api.ERP.sin.requestsinhc(params)
        if (res.code == 1) {
