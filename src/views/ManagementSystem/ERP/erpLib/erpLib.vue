<template>
  <div class="erp-lib">
    <div class="top" layout="row" layout-align="start center">
      <icon-button @click.native="addOrUpdateErpLibHandle(1)" text="新增erp仓库" img="btn_add.png" colors="#383943"></icon-button>
    ...
    <div class="table-content">
      <div class="table">
        ...
        <!-- 新增或编辑erp仓库 -->
        <drawerAddOrUpdateErpLib v-model="showDrawer" :currentInfo="currentInfo" :type="type" @getTableData="getTableData" />
        <!-- 置为有效无效删除冲突 -->
...
import drawerAddOrUpdateErpLib from './erpLibCom/drawerAddOrUpdateErpLib.vue'
import drawerNextDrawer from './erpLibCom/drawerNextDrawer.vue'
export default {
  data() {
    return {
      tableData: [],
  ...
  methods: {
    async getTableData() {
      try {
        const res = await this.$api.ERP.store.requeststorelist();
        if (res.code == 1) {
    ...
    async setEffectOrNotEffect(type){
      ...
      try {
        const api = type == 1 ? 'requeststoredisable' : (type == 2 ? 'requeststoreenable' : 'requeststoredel')
