<template>
  <div class="erp-lib">
    <div class="top" layout="row" layout-align="start center">
      <icon-button @click.native="addOrUpdateErpLibHandle(1)" text="新增供应商" img="btn_add.png" colors="#383943"></icon-button>
    ...
    <div class="table-content">
      <div class="table">
        ...
        <!-- 新增或编辑erp仓库 -->
        <drawerAddOrUpdateSupplier v-model="showDrawer" :currentInfo="currentInfo" :type="type" @getTableData="getTableData" />
        <!-- 置为有效无效删除冲突 -->
        <drawerNextDrawer v-model="showNextDrawer" :nexDrawerInfo="nexDrawerInfo" :type="type" @getTableData="getTableData" />
      </div>
    </div>
  </div>
</template>
...
import drawerAddOrUpdateSupplier from './supplierConfigCom/drawerAddOrUpdateSupplier.vue'
import drawerNextDrawer from './supplierConfigCom/drawerNextDrawer.vue'
export default {
  data() {
    return {
      tableData: [],
  ...
  methods: {
    async getTableData() {
      try {
        const res = await this.$api.ERP.supplier.requestsupplierlist();
        if (res.code == 1) {
    ...
    async setEffectOrNotEffect(type){
      ...
      try {
        const api = type == 1 ? 'requestsupplierdisable' : (type == 2 ? 'requestsupplierenable' : 'requestsupplierdel')
