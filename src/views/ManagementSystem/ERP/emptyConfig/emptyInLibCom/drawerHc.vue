...
export default {
  ...
  methods: {
    async getTableData(){
      const params = {
        id: this.currentInfo.id * 1 // int64  订单Id
      }
      try {
        const res = await this.$api.ERP.emptyLib.getHCOrderInLib(params);
        if (res.code == 1) {
    ...
    async onSubmit (){
      ...
      try {
        const res = await this.$api.ERP.emptyLib.submitHCInLib(params);
        if (res.code == 1) {
