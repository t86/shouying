  ...
  <div class="empty-lib-order">
    <div class="top">
      ...
      <div class="m-t-2" layout="row" layout-align="start center">
        <span class="label fs14">供应商：</span>
        <el-select v-model="gysVal" size="small" style="width:120px">
          <el-option label="全部" :value='0'></el-option>
          <el-option v-for="(item,i) in gysOption" :key="i" :label="item.n" :value="item.id"></el-option>
        </el-select>
  ...
  methods: {
    async getOptions(){
      try {
        ...
        if(res.code == 1) {
          ...
          this.gysOption = res.data.suppliers || []
    ...
    async getTableData() {
      const params = {
        ...
        begin_day: this.dateVal[0], //  string  入库开始日期 格式  yyyy-mm-dd
        end_day: this.dateVal[1], //    string  入库结束日期 格式 yyyy-mm-dd
        store_id: this.inLibVal * 1, //   int64   入库仓库 ,0 表示不限制
        ...
        supplier_id: this.gysVal * 1, // int64  供应商id, 0 代表不限制
        name: this.remark || '', //       string   模糊查询关键字, 物料名称或简称 , 空, 表示不限制
      };

      try {
        const res = await this.$api.ERP.rpt_erp.requestrpt_erpin_recs(params);
        if (res.code == 1) {
    ...
    async exportExcelHandle(){
      ...
      try {
        const res = await this.$api.ERP.rpt_erp.reqExportInLibRpt(params)
        if (!res.msg) {
