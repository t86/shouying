  ...
  methods: {
    ...
    async getTableData() {
      const params = {
        ...
        in_store_id: this.inLibVal * 1, //   int64   入库仓库 ,0 表示不限制
      ...
      try {
        const res = await this.$api.ERP.rpt_erp.requestrpt_erpin_sum(params);
        if (res.code == 1) {
    ...
    async exportExcelHandle(){
      ...
      try {
        const res = await this.$api.ERP.rpt_erp.reqExportInLibAllRpt(params)
        if (!res.msg) {
