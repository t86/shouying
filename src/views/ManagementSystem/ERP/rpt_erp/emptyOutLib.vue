...
export default {
  ...
  methods: {
    async getTableData() {
      ...
      try {
        const res = await this.$api.ERP.reportEmpty.reqGetEmptyOutLibDetailList(params);
        if (res.code == 1) {
    ...
    async exportExcelHandle(){
      ...
      try {
        const res = await this.$api.ERP.reportEmpty.reqExportEmptyOutLibDetailList(params)
        if (!res.msg) {
