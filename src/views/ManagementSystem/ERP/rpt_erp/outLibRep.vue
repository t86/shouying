...
export default {
  ...
  methods: {
    ...
    async getTableData() {
      ...
      try {
        const res = await this.$api.ERP.rpt_erp.requestrpt_erpout_recs(params);
        if (res.code == 1) {
    ...
    async exportExcelHandle(){
      ...
      try {
        const res = await this.$api.ERP.rpt_erp.reqExportOutLibRpt(params)
        if (!res.msg) {
