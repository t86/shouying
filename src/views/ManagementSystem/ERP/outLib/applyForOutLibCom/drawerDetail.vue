<template>
  <div>
    ...
    >
      ...
      <div class="tips red-color p-l-6">
        <h5 class="m-b-2">说明：</h5>
        <p class="fs14 m-b-2">1、异议红冲：异议数量直接红冲（适用情形：实际出库8瓶，但出库单数量填写成10瓶）</p>
        <p class="fs14">2、异议报损：异议数量自动生成其他出库（报损）单（适用情形：实际出库10瓶，出库单数量也为10瓶，但是有2瓶破损）</p>
      </div>
      ...
      <div class="form-btn" layout="row" layout-align="center center">
        ...
        <el-button type="primary" v-if="info.s == '待处理'" @click="applyForHcHandle">异议红冲</el-button>
        <el-button type="primary" v-if="info.s == '待处理'" @click="applyForHandle">异议报损</el-button>
        <el-button type="primary" v-if="info.s == '待处理'" @click="backOrderHandle">驳回</el-button>
      </div>
    </el-drawer>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    // 异议红冲
    async applyForHcHandle(){
      const params = {
        id: this.currentInfo.id * 1
      }
      try {
        const res = await this.$api.ERP.soutdd_yy.requestd_yyout_hc(params)
        if(res.code == 1) {
        ...
        console.log('异议红冲失败', error);
      }
    },

    // 异议报损
    async applyForHandle(){
      const params = {
        id: this.currentInfo.id * 1
      }
      try {
        const res = await this.$api.ERP.soutdd_yy.requestd_yyout_bs(params)
        if(res.code == 1) {
          this.$message.success('操作成功')
          this.onCancelDrawer()
          this.$emit('getTableData')
