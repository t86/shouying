          ...
          <div class="table">
            ...
            <div class="tbody">
              <div class="tr" layout="row" layout-align="space-between center" v-for="(item, i) in tableData" :key="item.id">
                ...
                <div class="td">{{((item.rp/item.c) * item.hcCount).toFixed(2)}}</div>
                <div class="td">{{((item.sp/item.c) * item.hcCount).toFixed(2)}}</div>
              </div>
...
export default {
  ...
  methods: {
    ...
    async getAllData(){
      ...
      try {
        const res = await this.$api.ERP.soutd.requestsoutdget_f_hc(params)
        if(res.code == 1) {
          this.info = res.data || {}
    ...
    async onSubmit (){
      ...
      if(resultTableData.length <= 0) return this.$message.warning('请填写红冲数量')
      const params = {
        record_dtl_ids: resultTableData.map(item => item.id * 1),
        cnts: resultTableData.map(item => item.hcCount * 1)
      }
      try {
        const res = await this.$api.ERP.soutd.requestsoutdhc(params)
        if (res.code == 1) {
