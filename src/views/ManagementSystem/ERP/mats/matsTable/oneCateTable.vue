<template>
  <div class="one-table">
    <div class="top" layout="row" layout-align="start center">
      <icon-button @click.native="addOrUpdateOneCateHandle(1)" text="新增" img="添加.png" colors="#383943"></icon-button>
      <icon-button @click.native="addOrUpdateOneCateHandle(2)" text="编辑" img="编辑.png" colors="#383943"></icon-button>
...
export default {
  ...
  methods: {
    async getTableData() {
      try {
        const res = await this.$api.ERP.mocate.requestmocatelist();
        if (res.code == 1) {
    ...
    async setEffectOrNotEffect(type){
      ...
      try {
        const api = type == 1 ? 'requestmocatedisable' : (type == 2 ? 'requestmocateenable' : 'requestmocatedel')
