  ...
  <div class="menu-list">
    <div class="nav">
      ...
      <div class="nav-menu" :style="{ width: navWidth + 'px' }">
        ...
        <div class="menu-list">
          ...
          <ul v-if="!searchVal" class="first-cate">
            ...
            <secondMenu v-if="showMenuList" :menuList="menuList" />
          </ul>
          <!-- 展示搜索结果的二级菜单 -->
      ...
      <div class="nav-content" :style="{ 'margin-left': navWidth + 10 + 'px' }">
        <matOneTable v-if="menuIndex == 0" @getMenuList="getMenuList" />
        <matTwoTable v-if="menuIndex == 1" :menuList="menuList" @getMenuList="getMenuList"/>
        <prdTable v-if="menuIndex == 2" :menuList="menuList" @getMenuList="getMenuList" />
        <!-- 新增或编辑 -->
        <addOrUpdatePrd v-model="showDrawer" :prdId="prdId" type="2" :menuList="menuList" search="true" @initSearchValHandle="initSearchValHandle" />
      </div>
    </div>
  </div>
</template>
...
<script>
const menuWidth = 230;
import matOneTable from "./matsTable/oneCateTable.vue";
import matTwoTable from "./matsTable/twoCateTable.vue";
import prdTable from './matsTable/prdTable.vue';
import addOrUpdatePrd from './matsTable/prdTableCom/drawerAddOrUpdatePrd.vue'
export default {
  data() {
    return {
      navWidth: menuWidth,
  ...
  methods: {
          ...
          this.menuList = (res.data || []).map(item => {
            ...
            if (isSearch) {
              if(this.menuIndex == 1) {
                ...
                // 搜索二级菜单
                const findTwoCate = item.subs.find(items => items.id == menuId)
    ...
    getSearchDetail(itemInfo, type) {
      if(type == 'oneCate' || type == 'twoCate') {
        ...
        this.$router.push('/ERP/mats?menuIndex=' + menuIndex + '&menuId=' + menuId + '&search=true')
        this.searchVal = "";
