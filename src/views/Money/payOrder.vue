<template>
  <div class="pay-order">
    ...
    <div class="order-content">
      ...
      <!-- 底部数据 -->
      <div class="order-content-bottom" layout="row" layout-align="space-between center">
        <div
        ...
        >
          ...
          >
            <span>未结账金额:</span>
            <span class="amt">
              ¥{{turnOverInfo.activeTurnOverCount == turnOverInfo.turnOverTabList.length - 1 ? ($store.state.orderInfo.currentCardInfo.orderAmt - $store.state.orderInfo.currentCardInfo.payedAmt).toFixed(2) : '0.00'}}
            </span>
          </div>
        ...
        >
          ...
          <div
            ...
            v-if="payTabInfo.activePayId==0"
            @click="togglePayType"
          >设为{{$store.state.orderInfo.currentCardInfo.settle_mode == 1 ? '后' : '先' }}买单</div>
          <!-- 并台（转优惠2） -->
          <div
            class="button"
            :class="{'gray': hasChooseClockOrder}"
...
export default {
  ...
  methods: {
    ...
    getPayTabList() {
      ...
      // 判断是否为翻台后的空台状态进来查看详情
      const payTabCount = this.$store.state.orderInfo.currentCardInfo.bizStatus == 1 ? this.turnOverInfo.turnOverTabList.length : this.turnOverInfo.turnOverTabList.length - 1

      this.cardAllOrderInfo.forEach((el, i) => {
        if(!(this.turnOverInfo.activeTurnOverCount < payTabCount && el.pid == 0)) {
          payTabList.push({
    ...
    getPayedOrderInfo(payedId) {
      ...
      const payedInfo = this.cardAllOrderInfo.find(item => item.pid == payedId);
      // payedId == -1 ==> 全部订单
    ...
    async changeTab(type, id, isReset = true) {
      switch (type) {
        case "turnOverCount": // 点击翻台tab
          if (id == -2) {
            return (this.turnOverInfo.turnOverTabShow = !this.turnOverInfo
              .turnOverTabShow);
          }
    ...
    // 切换先后买单方式
    async togglePayType(){
      if (this.hasChooseClockOrder) return this.$message.warning('所选订单存在锁定订单，不可操作')
      const params = {
        seat_id : this.$store.state.orderInfo.currentCardInfo.seatId * 1, //   int64   待操作卡台Id
        settle_mode: this.$store.state.orderInfo.currentCardInfo.settle_mode * 1 == 1 ? 2: 1, // int  1 先买单 2 后买单
      }
      try {
    ...
    mergeOrderToAnotherSeat() {
      ...
      if (!isYh2) return this.$message.warning("只允许优惠2订单进行转优惠2操作！");
    ...
    showOrHidePayDrawer() {
      ...
      this.notPayData.choosePayOrderList.forEach(el => {
        ...
        el.back || el.at==2 || el.at==3 ? amt += 0 : (amt += el.pp * 1 == 0 ? el.pa * 1 : el.changeCount * el.pp)
      })
    ...
    showOrHideBackDrawer() {
      this.drawer.orderBackDrawer.showDrawer = !this.drawer.orderBackDrawer.showDrawer;
    },
        ...
        this.$nextTick(() => {
          if(downKeyCode[0] == 17 && downKeyCode[1] == 81) {
            ...
            if(this.payTabInfo.activePayId == 0) {
              // 未付款订单
              this.$refs.notPayOrder.checkoutAll = !this.$refs.notPayOrder.checkoutAll
            ...
            // shift + s  // 优惠2
            e.preventDefault()
            this.$router.push({name:'orderMealList', query:{give:true}})
            
          } else if (downKeyCode[0] == 0 && downKeyCode[1] == 13) {
            // enter
            e.preventDefault()
            if(this.payTabInfo.activePayId == 0 && this.$refs.notPayOrder.drawer.showDrawer) {
              // 未支付订单套餐更改明细
              this.$refs.notPayOrder.$refs.drawerMyOrder && this.$refs.notPayOrder.$refs.drawerMyOrder.onSubmit()
            } else if (this.payTabInfo.activePayId > 0 && this.$refs.payedOrder.showUpdateDetailDrawer) {
              // 已支付订单套餐更改明细
              this.$refs.payedOrder.$refs.drawerMyOrder && this.$refs.payedOrder.$refs.drawerMyOrder.onSubmit()
            } else if (this.payTabInfo.activePayId == 0 && !this.drawer.payDrawer.showDrawer) {
              // 未付款订单收款列表
              this.showOrHidePayDrawer()
            } else if (this.payTabInfo.activePayId == 0 && this.drawer.payDrawer.showDrawer && !this.$refs.drawerPayMoney.showAuthDrawer) {
              // 未付款订单确认收款
              this.$refs.drawerPayMoney.onSubmit()
            } else if (this.payTabInfo.activePayId == 0 && this.$refs.drawerPayMoney.showAuthDrawer) {
              // 确认添加支付渠道授权
              this.$refs.drawerPayMoney.$refs.authComponent.onSubmit()
            } else if (this.payTabInfo.activePayId != 0 && this.$refs.payedOrder.showDrawer) {
              // 修改支付渠道
              if (this.$refs.payedOrder.$refs.drawerUpdatePayedType.showAuthDrawer) {
                // 添加授权支付渠道授权授权账号密码
                this.$refs.payedOrder.$refs.drawerUpdatePayedType.$refs.drawerPayMoneyAuth.onSubmit()
              } else {
                // 提交更改授权渠道
                const drawerUpdatePayedTypeCom = this.$refs.payedOrder.$refs.drawerUpdatePayedType
                if (drawerUpdatePayedTypeCom) drawerUpdatePayedTypeCom.onSubmit()
              }
            ...
            } else if (this.payTabInfo.activePayId == -1 && this.showPrintDrawer) {
              // 打印消费单（未结/全部）
            ...
            if (this.drawer.payDrawer.showDrawer) {
              ...
              if(this.$refs.drawerPayMoney.showAuthDrawer){
                // 支付抹零授权
                const authCom = this.$refs.drawerPayMoney.$refs.authComponent.$refs.authorization
                authCom.tabIndex = 1
              }
            } else {
              // 左切换付款次数
              // 其它tab的索引
              const anotherIndex = this.payTabInfo.payTabList.findIndex(item => item.id == -2)
              const resultPayTabList = this.payTabInfo.payTabList.filter(item => item.id != -2)
              let index = resultPayTabList.findIndex(item => item.id == this.payTabInfo.activePayId)
            ...
            // up箭头  // 向上选择支付方式
            e.preventDefault()
            if (this.payTabInfo.activePayId == 0 && this.drawer.payDrawer.showDrawer) {
              // 付款
              if (this.$refs.drawerPayMoney.showAuthDrawer) {
                // 授权向上选择输入username
                const authCom = this.$refs.drawerPayMoney.$refs.authComponent.$refs.authorization
                ...
                // 向上选择支付方式渠道
                let index = this.$refs.drawerPayMoney.payList.findIndex(item => item.id == this.$refs.drawerPayMoney.payActiveInfo.id)
                index = Math.max(0, index - 1)
                this.$refs.drawerPayMoney.payActiveInfo = this.$refs.drawerPayMoney.payList[index]
              }
              
            } else if (this.payTabInfo.activePayId != 0 && this.$refs.payedOrder.showDrawer) {
              // 修改支付渠道
              const com = this.$refs.payedOrder.$refs.drawerUpdatePayedType && this.$refs.payedOrder.$refs.drawerUpdatePayedType.$refs.drawerAddPayDetail
              if (com) {
                let index = com.payList.findIndex(item => item.id == com.payActiveInfo.id)
                index = Math.max(0, index - 1)
            ...
            if (this.payTabInfo.activePayId == 0 && this.drawer.payDrawer.showDrawer) {
              ...
              if (this.$refs.drawerPayMoney.showAuthDrawer) {
                ...
                // 向下选择支付方式渠道
                let index = this.$refs.drawerPayMoney.payList.findIndex(item => item.id == this.$refs.drawerPayMoney.payActiveInfo.id)
                index = Math.min(this.$refs.drawerPayMoney.payList.length - 1, index + 1)
                this.$refs.drawerPayMoney.payActiveInfo = this.$refs.drawerPayMoney.payList[index]
              }
            } else if (this.payTabInfo.activePayId != 0 && this.$refs.payedOrder.showDrawer) {
              // 修改支付渠道
  ...
  computed: {
    hasChooseClockOrder(){
      return this.notPayData.choosePayOrderList.filter(item => !item.oid).some(item => item.l > 0)
    },
    hasYh2Order(){
      return this.cardAllOrderInfo && this.cardAllOrderInfo[this.cardAllOrderInfo.length - 1] && this.cardAllOrderInfo[this.cardAllOrderInfo.length - 1].list && this.cardAllOrderInfo[this.cardAllOrderInfo.length - 1].list.some(item => item.y2s)
    },
    // 已结账翻台记录总金额
    turnOverAllAmt(){
      let amt = 0 
      if(this.turnOverInfo.activeTurnOverCount < this.turnOverInfo.turnOverTabList.length - 1) {
