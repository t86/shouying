<template>
  <div class="card-box">
        ...
        <div class="content">
          ...
          >
            <div
              ...
              :key="index"
              :class="[{'opacity':typeModule == 1 && (item.bizStatus==1||item.bizStatus==2||item.bizStatus==8)}, 'bgc'+(Number(item.bizStatus))]"
            ...
            >
              <p layout="row" layout-align="space-between center">
                ...
                <span>
                  ...
                  <span v-if="typeModule == 1">{{item.openTime ? '(' + item.openTime + ')' : '' | filterTime}}</span>
                </span>
              </p>
              ...
              <p
                style="height:18px"
                layout="row"
                v-if="typeModule == 1 && item.bizStatus!=1&&item.bizStatus!=2&&item.bizStatus!=8"
              ...
              >
                ...
                <span
                  v-if="item.canLookOrder"
                  class="card-step"
                >{{item.diXiaoJindu}}</span>
              </p>
              ...
              <p
                style="height:18px"
                v-if="typeModule == 1 && item.canLookOrder && item.bizStatus!=1&&item.bizStatus!=2&&item.bizStatus!=8&&$store.state.userInfo.authStatusArr.includes(2)"
              >
              ...
              <p layout="row" layout-align="space-between center" style="margin-top:4px;">
                ...
                <!-- 订位人 -->
                <span v-else-if="typeModule == 1 && item.bizStatus!=1 && item.bizStatus!=2" class="one-txt-cut" style="width:110px">
                ...
                <!-- 翻台数 -->
                <span v-if="item.turnoverCnt>0 && ($store.state.userInfo.authStatusArr.includes(1) || $store.state.userInfo.authStatusArr.includes(2))">
                  翻{{item.turnoverCnt}}
                </span>
              </p>
            </div>
          ...
          <div v-else class="no-data">
            ...
            <p>{{ tab.activeIndex==0 ? '暂无服务过的卡台，快去点单吧！' : '暂无卡台数据'}}</p>
          </div>
        </div>
        <!-- 卡台图例tab -->
        <div class="legend" layout="row" layout-align="space-between center">
          ...
          <div class="legend-list" layout="row" layout-align="space-around center">
            ...
            <div class="legend-list-item" v-for="(item,index) in legendList" :key="item.id">
              ...
              <span class="btn" style="transform:translateY(26px)" v-else :class="{'active': legendActive===item.id}">
                <div class="img-contain" :class="[item.id == 20 ? '' : 'bgc' +(index+2)]">
          ...
          <div class="dosomething" layout="row" layout-align="space-between center">
            ...
            <div class="dosomething-item checkout-btn" v-show="$store.state.userInfo.sys_modules && $store.state.userInfo.sys_modules.includes(1)" layout="column" layout-align="center center" @click.stop="checkOutModuleHandle">
              <img :src="require('@/assets/card-imgs/checkout.png')" alt />
              <p class="m-t-1">{{typeModule == 1 ? '存酒模式' : '点单模式'}}</p>
            </div>
    ...
    <updateAuthPassword 
      ...
      :showDrawer="legendOptions.showAuthPwdModel" @submitHandle="submitUpdateAuthPwdHandle" @cancelUpdatePwdHandle="legendOptions.showAuthPwdModel=false" />
...
export default {
  ...
  methods: {
    ...
    getTabList(arr = []) {
      ...
      if(!(authStatusArr.includes(1) || authStatusArr.includes(2) || authStatusArr.includes(3))) {
        // 无任何权限
        arr = []
      } else {
        arr = [...FUTabList, ...YXTabList, ...HLTabList].filter((item, index, arr) => arr.findIndex(items => items.id == item.id) == index)
      }

      const tabList = arr.sort((a, b) => a.dsp - b.dsp).filter(item => item.status == 1); // status:  1:有效 2:无效
      this.tab.tabListOrigin = JSON.parse(JSON.stringify(tabList));
      this.$store.commit("updateTabList", this.tab.tabListOrigin);

      tabList.unshift({
        id: 0,
        name: authStatusArr.length == 1 && authStatusArr.includes(1) ? '我的卡台' : "全部"
      });
    ...
    getAuthArea(arr) {
      ...
      const authStationArea = this.$store.state.cardPageInfo.resResultDataObj.stationArea.filter(
        el => el.status == 1
      );
      ...
      canGetOrderAreaList = arr.filter(item => configAreaIdList.find(items => items == item.id))

      const tabList = [...canPayOrderAreaList, ...canGetOrderAreaList]
      return tabList.filter((item, i, array) => array.findIndex(items => items.id == item.id) == i);
    },
    ...
    async getCardList(cardInfo = [], businessData = []) {
      ...
      if((this.$store.state.userInfo.authStatusArr.includes(1) || this.$store.state.userInfo.authStatusArr.includes(2)) && !this.modelVisible) {
        // 服务员/营销  且已开启营业日
      ...
      try {
        ...
        const index = this.tab.tabList.findIndex(item => item.id == this.tab.anotherInfoActiveId)
    ...
    getCardOptions(bizStatus, turnoverCnt){
      ...
      if(turnoverCnt <= 0) {
        // 未过翻台
        if(bizStatus == 4 || bizStatus == 5 || bizStatus == 6 || bizStatus == 7) {
          cardOption = [1, 2, 3]
        } else {
          cardOption = [3]
        }
      ...
      return cardOption.map(item => orderSaveWineOptions.find(items => items.id == item))
    },

    // 我的卡台数据(卡台列表)
    getMyCardList() {
      const authEmpId = this.$store.state.userInfo && this.$store.state.userInfo.emp_id;
    ...
    filterCardList(key, id) {
      ...
      // 登录账号身份筛选
      if (this.$store.state.userInfo.authStatusArr.length == 1 && this.$store.state.userInfo.authStatusArr.includes(1)) {
        // 单纯只有服务员权限
        return id == 0 ? this.getMyCardList() : cardListInfoArr.filter(item => item[key] == id);
      } else {
        // 有除了服务员以外的多个权限
        return id == 0 ? JSON.parse(JSON.stringify(cardListInfoArr)) : cardListInfoArr.filter(item => item[key] == id);
      }
    },
    ...
    getSelfStaffList(selfId) {
      ...
      const allStaffList = this.$store.state.cardPageInfo.resResultDataObj[
        "orderPersonInfo"
      ];
      // 当前员工的下级
    ...
    async addOwnPayedAmtToCard(cardList){
      ...
      return cardList.map(item => {
        const OwnPayInfo = ownPayedCardList.find(items => items.s == item.seatId)
        return {
          ...item,
    ...
    currentCardCanLookOrder(n,cardItemInfo){
      ...
      // 是否是当前卡台订位人
      const isBooker = cardItemInfo.salesEmpId * 1 == this.$store.state.userInfo.emp_id * 1
      // 是否是当前卡台点单服务员
      const isSealer = !!cardItemInfo.waiter_emp_ids_arr.find(item => item * 1 == this.$store.state.userInfo.emp_id * 1)
    ...
    setLegendCount(regionId = 0){
      setTimeout(() => {
        ...
        if(isNoLimit.length > 0) {
          ...
          if(regionId == 0) {
            // 选择的'全部'
            if(this.$store.state.userInfo.authStatusArr.length == 1 && this.$store.state.userInfo.authStatusArr[0] == 1) {
              // 服务员，显示的我的卡台
