<template>
  ...
  <div class="my-order">
    <div class="my-order-content" :class="{rect: !isRect}">
        ...
        <div class="tbody">
          <div class="coll" v-for="item in orderList" :key="item.id" :class="{'online': item.io==1}">
            <div class="detail tr">
              ...
              <div class="td" :class="{'opacity':item.back}">{{item.pp}}</div>
              <div class="td" :class="{'opacity':item.back}">{{item.at == 2 || item.at == 3 ? '0.00' : item.pa}}</div>
              <div class="td" :class="{'opacity':item.back}">{{item.personInfo && item.personInfo.name}}</div>
              <div class="td" :class="{'opacity':item.back}">{{item.ot.slice(7)}}</div>
              <div class="td" :class="{'opacity':item.back}">{{item.authInfo? item.authInfo.name : '---'}}</div>
              <div class="td">
                <!-- <div class="bg" v-if="item.showList" @click="showOrHideList(item)"></div> -->
                <img
                  v-if="item.s!=5&&!item.back||item.productInfo.prdType==2"
            ...
            <div v-if="(item.r||item.is==1)&&!item.back" :class="{'pt':item.is==1}" class="requested">{{item.r}}</div>
            <!-- 补单信息 -->
      ...
      <div class="my-order-content-bottom" layout="row" layout-align="space-between center">
      ...
      try {
        ...
        if (res.code === 1) {
          ...
          data.forEach(el => {
            ...
            el.personInfo = el.wei == 0 ? {name: '自助'} : common_book.getOrderPersonInfo(el.wei);
            el.authInfo = common_book.getOrderPersonInfo(el.ae);
            el.showList = false;
            if (el.is == 1)
              el.si.forEach(ele => {
                ele.groupInfo = common_order.getProductInfoFromGroup(
                  ele.i
                );
              });
          });
