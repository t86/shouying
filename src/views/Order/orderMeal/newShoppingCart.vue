...
export default {
  ...
  methods: {
    ...
    calculateOrderItemAmount(item) {
      return Number(this.getSubtotal(item));
    }
  },
  ...
  computed: {
    ...
    canOrder() {
      ...
      if (
        ...
        // 第一次下单，判断是否点了必点商品
        return this.mustOrderPrdIds.every(item => this.shoppingCartList.map(item => item.pid).includes(item *1))
      } else {
        return true;
      }
    },
