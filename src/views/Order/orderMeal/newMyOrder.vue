<template>
  <div>
    ...
    <div class="new-my-order">
      ...
      <!-- 赠送、自用、退单等drawer -->
      <drawerMyOrder ref="drawerMyOrder" :status="drawer.status" :showDrawer="drawer.showDrawer"
        :currentItemInfo="drawer.currentItemInfo" :orderList="orderList" @showOrHideDrawer="showOrHideDrawer" />

      <!-- 打印消费单 -->
      <drawerPrintOrder ref="drawerPrintOrder" pageType="myOrder" :showDrawer="showPrintDrawer"
    ...
    async getOrderedData() {
      try {
        ...
        if (res.code === 1) {
          ...
          this.orderPayInfo = res.data.pay_info || {};
    ...
    applyPermissionFilter(resultData) {
      ...
      for (let i = 0; i < resultData.length; i++) {
        ...
        if (userInfo.roleIds && userInfo.roleIds.includes(11)) {
          console.log("督查角色处理订单:", el);
          
          // 1.1 检查是否有全场查单权限
          if (this.hasFullLookupPermission()) {
            console.log("督查有全场权限，添加订单");
        ...
        console.log("applyPermissionFilter - supervisor check:", {
          currentSeatId,
          seatId,
          orderRegionId, 
          ...
          // 如果没有区域权限，只能看自己的点单
          if (el.wei == userInfo.emp_id) {
            console.log("督查自己的订单，添加");
            authList.push(el);
          } else {
            console.log("督查无权限查看此订单，跳过");
          }
          continue;
        }
      ...
      console.log("applyPermissionFilter 最终返回结果:", authList);
      return authList;
    },
