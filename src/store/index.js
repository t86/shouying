...
export default new Vuex.Store({
  state: {
    ...
    orderInfo: {
      // 当前正在点单的卡台信息
      currentCardInfo: sessionStorage.getItem('currentCardInfo') ? JSON.parse(sessionStorage.getItem('currentCardInfo')) : {},
      saveWineCardInfo: sessionStorage.getItem('saveWineCardInfo') ? JSON.parse(sessionStorage.getItem('saveWineCardInfo')) : {},
      productAll: sessionStorage.getItem('productAll') ? JSON.parse(sessionStorage.getItem('productAll')) : [],  // 所有可点商品列表
      allProductsList: sessionStorage.getItem('allProductsList') ? JSON.parse(sessionStorage.getItem('allProductsList')) : [],  // 所有有效商品
      shoppingCartInfo: {
        shoppingCount: 0
      }
    },
  ...
  mutations: {
    ...
    updateResResultDataObj(state, value) {
      ...
      if (currentCardId) {
        const updatedCardInfo = value.businessData.find(el => el.seatId == currentCardId)
        for (let key in updatedCardInfo) {  // 更新当前卡台最新数据
          state.orderInfo.currentCardInfo[key] = updatedCardInfo[key] || state.orderInfo.currentCardInfo[key]
        }
        sessionStorage.setItem('currentCardInfo', JSON.stringify(state.orderInfo.currentCardInfo))
      }
    },
  ...
  actions: {
    ...
    async getShoppingCount(context, that) {
      try {
        ...
        res.code === 1
          ? (context.state.orderInfo.shoppingCartInfo.shoppingCount = res.data || 0)
