...
export default class WebSocketClient {
    ...
    connect = ()=> {
        typeof (WebSocket) === "undefined" ? this.vue.$message.warning("您的浏览器不支持socket") : this.connectWebsocket()
    }
    ...
    connectWebsocket = () => {
      ...
      errorHandle= (e) => {
        ...
        this.errorTimer = setTimeout(() => {
          ...
          this.websocketTimeMessageTime ? this.getUpdateData() : this.getAllData(true, false)
        }, 5000)
      }
    ...
    websocketHasConnect= () => {
      ...
      getAllData = async (loadCard = false, reload = false, init = false) => {
        ...
        try {
          ...
          if (res.code == 1 || res.code == 2) {
            this.websocketTimeMessageTime = res.data.ts ? res.data.ts : this.websocketTimeMessageTime
          }
          if (res.code == 1) {
            if(needReloadData) {
              ...
              resResultDataArr.forEach((el, index) => {
                this.resResultDataObj[el] = el != 'businessData' ? transformCardDataHandle(data[index], index) : transformCardDataHandle(data[index], index).filter(item => item.bizStatus != 22 && item.bizStatus != 33)
              })
    
              // 获取设备可操作区域或卡台
              const currentMachineId = this.vue.$localStorage.getItem("machineId")
              const currentAreaAndCardList  = (this.resResultDataObj['machineArea'] || []).filter(item => item.license_id == currentMachineId && item.status == 1)
              const isNoLimit = currentAreaAndCardList.filter(item => item.type_id == 3)
              if(isNoLimit.length <= 0) {
                // 有限制
                const areaList = currentAreaAndCardList.filter(item => item.type_id == 1) 
                const cardList = currentAreaAndCardList.filter(item => item.type_id == 2)
                // 当前配置的区域id
                ...
                areaIdList.forEach(el => {
                  // 存储配置区域的区域下所有卡台
                  const currentAreaCardList = allCardInfo.filter(item => item.regionId == el)
                  resultCardList = [...resultCardList, ...currentAreaCardList]
                })
  
  
                cardList.forEach(el => {
                  const find = allCardInfo.find(item => item.id == el.region_o_seat_id)
                  if(find) {
                ...
                this.resResultDataObj.areaInfo = resultAreaList.map(item => ({
                  ...item,
                  isAllCard: !!areaList.map(item => item.region_o_seat_id).find(items => items == item.id)  // 是否是配置的可可显示的区域（如果配置的区域，则展现出的卡台为当前区域下所有卡台，否则为部分卡台，即通过部分卡台反推出来的区域）
                }))
              ...
              this.vue.$store.commit('updateResResultDataObj', this.resResultDataObj)
              this.vue.$store.commit('updateStoreStatusId', this.resResultDataObj['storeStatusInfo'][0]['wkday_id'])
            } else {
     ...
     getUpdateData = async() => {
        ...
        try {
          ...
          const data = res.data || {}
          if (res.code == 1 || res.code == 2) {
            this.websocketTimeMessageTime = data.ts ? data.ts : this.websocketTimeMessageTime
          }
    ...
    updateCardList = (dataObj = {}, time = '') => {
        ...
        for (let key in dataObj) {
          ...
          if (key == 14 && this.resResultDataObj[resResultDataArr[key]].length == 0) {
            ...
            dataObj[key].forEach(el => {
              ...
              // 查找当前改变的数据下标索引值
              const index = this.resResultDataObj[resResultDataArr[key]].findIndex(ele => {
                ...
                } else if (Number(key) <= 20) {  // 数据id字段为id
                  return ele.id == el.id
                ...
                } else if(Number(key) == 27) { // 角色关联花篮,小费商品权限
                  return ele.station_id == el.station_id
              ...
              if (index < 0 ) {
                this.resResultDataObj[resResultDataArr[key]][this.resResultDataObj[resResultDataArr[key]].length] = el
              } else {
                this.resResultDataObj[resResultDataArr[key]][index] = el
              }
  
              // 刷新收银系统订单数据
              if(this.vue.$route.name == 'payOrder' && key * 1 == 14 && this.vue.$store.state.orderInfo.currentCardInfo && dataObj[key].some(item => item.seatId == this.vue.$store.state.orderInfo.currentCardInfo.seatId)){
