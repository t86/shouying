  ...
  <div class="auth">
    <div class="tab">
      <div class="tab-top" layout="row" layout-align="center center">
        ...
        <span :class="{'active':tabIndex==2}" @click="tabIndex=2">账号</span>
      </div>
        ...
        <div class="tab-content-item" v-if="tabIndex==2">
          <div class="form">
            ...
            <!-- <div v-else @click="checkedIndex=1" :class="{'active': checkedIndex==1,'text':userName==''}" class="username">
              ...
              <input-select
              style="display:inline-block;width:260px;line-height: 42px;"
              :autoFocus="true"
              :value="userName"
...
export default {
  ...
  methods: {
    ...
    changeSealName(info) {
      ...
      console.log(info)
      this.emitData({ key: "userName", value: this.code });
    },
    selectAeBlurHandle() {
      this.sales_info_option = [];
    },
    changeNum(id) {
      if (id == 10) {
        ...
        const userName = this.userName.split("");
          ...
          this.inputSealName(this.userName)
      } else {
    ...
    emitData({ key, value }) {
      console.log("updateAuthorizationInfo",key, value)
    ...
    getCodeAndPwd(personType, empCardInfo, type){
      ...
      if(type == 1) {
        // 账号密码授权
        authEmpCode = personType == "self" ? "" : this.code
...
@import "../../../style/order/orderMeal/shoppingCart/authorization2.less";
