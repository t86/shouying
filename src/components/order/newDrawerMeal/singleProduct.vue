<template>
  <el-form label-position="right" @submit.native.prevent style="margin-top: 30px">
    ...
    <!-- 优惠理由 -->
    <el-form-item :class="{ 'm-b-2': orderMealStatus == 2 }" v-if="!isGQ && orderMealStatus == 2">
      <p class="youhui-title">请选择优惠理由：</p>
      <ul class="reason-list" layout="row" layout-align="start center">
        <li :class="{ active: formData.selectedInfo.name == item.name }" v-for="item in reasonList" :key="item.id"
    ...
    ">
      <el-form-item v-if="productInfo.requireInfo.length > 0" :class="{ 'm-b-2': orderMealStatus == 2 }">
        <div class="require-change" :class="{ text: requireText != '定制要求' }" @click="showOrHideRequireDrawer">
          {{ requireText }}
        </div>
      </el-form-item>
    </div>
    ...
    <!-- 提交优惠2 -->
    <drawerYH2Submit ref="drawerYH2Submit" :showDrawer="showYH2Drawer" :selectedProList="[{ ...productInfo, pc: count }]"
      @showOrHideDrawer="showOrHideYH2Drawer" @showOrHideAnotherDrawer="onCancelDrawer" />

    <!-- 定制化口味等要求 -->
    <drawerChooseRequireInfo :showDrawer="requireDrawerInfo.showDrawer" :productInfo="productInfo"
      :requestInfoArr="requestInfoArr" @updateRequireInfoArr="updateRequireInfoArr"
    ...
    <div class="modal" v-if="showModal">
      <div class="modal-content">
        ...
        <div v-if="productInfo.canOrderMeal">
          <el-radio class="m-t-3 m-l-3" v-model="modalRadio" label="1">点单下单</el-radio>
        </div>
        <div v-if="productInfo.canSeal">
          <el-radio class="m-t-3 m-l-3" v-model="modalRadio" label="2">优惠下单</el-radio>
        </div>
        <div v-if="productInfo.canHL">
          <el-radio class="m-t-3 m-l-3" v-model="modalRadio" label="3">特饮/小费下单</el-radio>
        </div>
        <div v-if="productInfo.canSealYH2">
          <el-radio class="m-t-3 m-l-3" v-model="modalRadio" label="5">优惠2下单</el-radio>
        </div>

        <div class="btn m-t-6" style="text-align: center">
          <el-button type="info" size="mini" class="m-r-4" style="background: #374368; border-color: #374368"
            @click.stop="onCancelDrawer">取消</el-button>
          <el-button type="primary" size="mini" @click.stop="modalSubmitHandle">确认</el-button>
        </div>
      </div>
    </div>
  </el-form>
...
@import "../../../style/order/orderMeal/newDrawerMeal/singleProduct.less";
