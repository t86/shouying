<template>
  <!-- 服务员添加滞留金（让客人扫码） -->
  <div>
    <el-drawer :title="title" :visible.sync="show" :before-close="notPayHandle" direction="rtl" append-to-body
      size="630px">
      <div class=" p-5 session  fs14">
        <div v-show="status == 1">
          <div class="coll" layout="row" layout-align="start center">
            ...
            <div class="value" layout="row" layout-align="start center">
              <div class="choose" :class="{ active: payType == item.id }" v-for="item in payTypeList" :key="item.id"
                ...
                <img :src="item.icon" style="height: 30px; width: 30px; margin-right: 8px; vertical-align: middle"
                  alt="" />
          ...
          <div class="coll" layout="row" layout-align="start center">
            ...
            <div class="value price" layout="row" layout-align="start center">
              <el-input v-model="amt" placeholder="请输入金额"></el-input>&nbsp;
              <span>元</span>
            </div>
          </div>
        ...
        <div v-show="status == 3 && qrResult == null">
          <div class="coll" layout="row" layout-align="start center">
            <div class="value" layout-align="start center">
              <el-input style="width: 260px" v-model="scanCode" size="small" placeholder="请输入支付码"></el-input>
            </div>
          </div>
          <div class="m-t-6">
      ...
      <div class="form-btn" layout="row" layout-align="center center">
        <el-button type="info" v-if="status == 1 || (status == 3 && qrResult == null)"
          @click="onCancelDrawer">取消</el-button>
        <el-button type="primary" v-if="status == 1 || (status == 3 && qrResult == null)" @click="onSubmit">确定</el-button>

        <div>
          <el-button type="info" v-if="status == 2 && ![5, 6].includes(payType)" @click="notPayHandle">暂不支付</el-button>
          <el-button type="primary" v-if="status == 2 && ![5, 6].includes(payType)"
            @click="goPrevStepHandle">上一步</el-button>
        </div>
        <div v-if="[5, 6].includes(payType)">
          <el-button type="info" v-if="status == 2 && qr_pay_state == 0" @click="notPayHandle">关闭</el-button>
          <el-button type="info" v-if="status == 2 && qr_pay_state == 2" @click="onCancelDrawer">关闭</el-button>
          <el-button type="info" v-if="status == 2 && qr_pay_state == 2" @click="goPrevStepHandle">重新扫描</el-button>
        </div>
      </div>
    </el-drawer>
  </div>
...
export default {
  ...
  components: {
    VueQr,
    keyBoard: () => import("@/components/common/newKeyBoard"),
  },
  watch: {
    value: {
      handler(newVal) {
