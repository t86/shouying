    ...
    >
      <div class="content">
        <div class="session m-b-10">
          ...
          <div class="info-detail m-t-6" layout="row" layout-align="space-between start">
            <div class="left">
              <div class="title m-b-3">散瓶数量：</div>
              <div class="table-content">
                <div class="table">
                  <div class="thead">
                    <div class="tr" layout="row" layout-align="space-between center">
                      <div class="th"><img @click="addRowHandle" :src="require('@/assets/order-img/add-count.png')"></div>
                  ...
                  <div class="tbody">
                    <div class="tr" layout="row" layout-align="space-between center" v-for="(item,index) in tableData" :key="item.id">
                      <div class="td"><img @click="deleteHandle(item.id)" :src="require('@/assets/order-img/delete.png')"></div>
                      <div class="td">
                        <el-select v-model="item.selectVal" size="mini" style="width: 80%" placeholder="请选择">
                          <el-option
                      ...
                      <div class="td" layout="row" layout-align="start center">
                        <input @click="focus=index;itemText='weight'" :class="{focus: focus == index&&itemText == 'weight'}" type="number" :min="1" v-model="item.allWeight" />克
                      </div>
                      <div class="td" layout="row" layout-align="start center">
...
export default {
  ...
  methods: {
    ...
    changeNumHandle(value) {
      if(this.focus == -1) {
        ...
        switch (value) {
          ...
          case 12: // 回退(
            this.chooseCount =
              this.chooseCount.toString().slice(0, this.chooseCount.toString().length - 1) *
              1;
            break;
          default:
            this.chooseCount = (this.chooseCount == 0 ? '' : this.chooseCount.toString()) + value * 1;
            break;
        }
      } else {
        ...
        if(this.itemText == 'weight') {
          switch (value) {
            ...
            case 12: // 回退(
              currentInfo.allWeight =
                currentInfo.allWeight.toString().slice(0, currentInfo.allWeight.toString().length - 1) *
                1;
              break;
            default:
              currentInfo.allWeight = currentInfo.allWeight.toString() + value * 1;
              break;
          }
          ...
          switch (value) {
            ...
            case 12: // 回退(
              currentInfo.count =
                currentInfo.count.toString().slice(0, currentInfo.count.toString().length - 1) *
                1;
              break;
            default:
    ...
    async onSubmit(){
      ...
      if(this.tableData.length > 0) {
        if(this.tableData.every(item => item.selectVal * 1 == 0)) return this.$message.warning('请选择商品规格')
        // if(this.tableData.every(item => item.allWeight * 1 == 0)) return this.$message.warning('请填写每瓶克数')
      }

      const resultList = this.chooseCount > 0 ? [{
        id: +new Date(),
        selectVal: '1',
        allWeight: 0,
  ...
  computed: {
    ...
    currentChooseCount(){
      return this.tableData.map(item => item.count * 1).reduce((a, b)=> a + b, 0) + this.chooseCount
    }
  },
  components: {
    keyBoard
  ...
  watch: {
    ...
    chooseCount(newVal){
      if(this.type == 1) {
        const currentMaxCount = this.maxCount - (this.tableData.map(item => item.count * 1).reduce((a, b) => a + b, 0))
        if(newVal < 0) {
          this.chooseCount = 0
