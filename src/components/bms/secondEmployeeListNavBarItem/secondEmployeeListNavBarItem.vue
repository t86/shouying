    ...
    <li class="item" v-for="(item,index) in menuList" :key="index">
      <div>
        ...
        >
          ...
          <div class="menu-name-contain" layout="row" layout-align="start center">
            <img src="@/assets/img/file_list.png" alt />
            <span class="name">{{item.n}}</span>
          </div>
        </div>
        <secondEmployeeListNavBarItem v-if="item.showList" :menuList="item.subs" />
      </div>
    </li>
  </ul>
</template>
...
@import "./secondEmployeeListNavBarItem.less";
</style>