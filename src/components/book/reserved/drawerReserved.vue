<template>
  <div>
    <el-drawer
      ...
      size="60%"
    >
      <el-form label-position="right" label-width="150px" :model="formData" ref="drawerRef" @submit.native.prevent>
        ...
        <el-form-item label="客人姓名" class="required required4">
          <el-input
...
export default {
  ...
  methods: {
     ...
     */
    async onSubmit() {
      const dateStr = this.$store.state.cardPageInfo.resResultDataObj.canDoList[this.dateTabActiveIndex].name
    ...
    resetForm() {
      ...
      this.formData.customPay.exp_min_csm_amt = this.$store.state.orderInfo.currentCardInfo.grpMinCsmAmt || ''; // 预期最低消费
    ...
    isValidParams(key) {
      if (this[key].customer_phone.length !== 0 && this[key].customer_phone.length != 11)
  ...
  computed: {
    addOrUpdateReservedParams() {
      ...
      return {
        ...
        book_day: this.$store.state.cardPageInfo.resResultDataObj.canDoList[this.dateTabActiveIndex].name.split('(')[1].split(')')[0], //   string     //BookDay 预定日期,格式 yyyy-mm-dd
