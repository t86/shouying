<template>
  <div>
    <el-drawer :title="title" :visible.sync="show" :before-close="closeDrawerHandle" direction="rtl" :size="size">
      <el-form @submit.native.prevent v-if="showType=='form'" label-position="right" label-width="150px" :model="formData" ref="drawerRef">
        <!-- 订位人预约信息 -->
        <div v-if="formData.sales.sales_status_arr.indexOf(formStatus)>-1">
          <el-form-item label="订位人">
            <input-select style="width:80%" :value="formData.sales.sales_name" placeholder="请输入姓名或工号"
              :optionsList="formData.sales.sales_info_option" @selectInputHandle="inputSealName"
              @selectOptionItem="changeSealName" @selectBlurHandle="selectBlurHandle"></input-select>
          </el-form-item>
          <el-form-item label="订位人电话">
            <el-input v-model="formData.sales.sales_phone" disabled style="width:80%;"></el-input>
          </el-form-item>
        </div>
        <!-- 卡台标记 -->
        <el-form-item v-if="formData.mark_status_arr.indexOf(formStatus)>-1" label="卡台标记">
          <form-select :value="formData.markInfo.value" style="width:80%" placeholder="请选择卡台标记"
            :optionsList="formData.markInfo.option" @selectOptionItem="changeTips"
            @selectBlurHandle="selectBlurHandle" @getOption="getMarkOption" />
            <i v-if="formData.markInfo.value" @click="formData.markInfo.value=''" class="el-icon-circle-close" style="position:absolute;right:23%;top:14px;color:rgba(255,255,255,.15);cursor:pointer"></i>
        </el-form-item> 

        <el-form-item
          v-if="formData.customArriveInfo.custom_arrive_status_arr.indexOf(formStatus)>-1&&((formStatus==2&&cardInfo.bizStatus==3)||formStatus==1||formStatus==8)"
          class="required required6" label="预定到达时间">
          <form-select :value="formData.customArriveInfo.exp_arrive_time" style="width:80%" placeholder="请选择预定到达时间"
            :optionsList="formData.customArriveInfo.exp_arrive_time_option" @selectOptionItem="changeTime"
            @selectBlurHandle="selectBlurHandle" @getOption="getArrivedTimeOption" />
        </el-form-item>

        <div v-if="formData.customPay.consumer_status_arr.indexOf(formStatus)>-1">
          <div v-if="showMinAmtOpenTypeArr.indexOf(formData.openType.open_type)>-1">
            <el-form-item label="预期最低消费" >
              <el-input v-model="formData.customPay.exp_min_csm_amt" type="number" style="width:80%" min="0"
                placeholder="请输入预期最低消费">
              </el-input>
              <span class="m-l-1 white">元</span>
            </el-form-item>
            <!-- 预定开台时显示卡台最低消费 -->
            <el-form-item v-if="formStatus==2&&cardInfo.bizStatus==3" label="卡台最低消费">
              <el-input v-model="formData.customPay.card_min_csm_amt" disabled type="number" style="width:80%">
              </el-input>
              <span class="m-l-1 white">元</span>
            </el-form-item>
          </div>
          <el-form-item label="预期预定人数" >
            <el-input v-model="formData.customPay.customer_exp_cnt" type="number" min="0" maxlength=100
              style="width:80%" placeholder="请输入预期预定人数" @input="inputCardCountHandle"></el-input>
          </el-form-item>
        </div>

        <!-- 开台类型 -->
        <div v-if="formData.openType.open_type_status_arr.indexOf(formStatus)>-1">
          <div v-if="formStatus==13">
            <el-form-item label="最低消费" v-if="showMinAmtOpenTypeArr.indexOf(formData.openType.open_type)>-1"
              class="required required4">
              <el-input v-model="formData.lowConsume.min_csm_amt" type="number" min="0" style="width:80%" placeholder="请输入最低消费">
              </el-input>
              <span class="m-l-1 white">元</span>
            </el-form-item>
            <el-form-item label="修改理由" class="required required4">
              <el-input type="textarea" :rows="2" placeholder="请输入理由" v-model="formData.openType.reason"
                style="width:80%">
              </el-input>
            </el-form-item>
          </div>
        ...
        <!-- 客人信息 -->
        <div v-if="formData.customInfo.consumer_info_status_arr.indexOf(formStatus)>-1">
          <el-form-item label="客人电话">
            <input-select style="width:80%" :value="formData.customInfo.customer_phone" placeholder="请输入客人电话"
              :optionsList="formData.customInfo.custom_info_option"
              @selectInputHandle="inputCustomPhone" @selectOptionItem="changeCustomPhone"
              @selectBlurHandle="selectBlurHandle"></input-select>
          </el-form-item>
          <el-form-item label="客人姓名">
            <el-input v-model="formData.customInfo.customer_name" style="width:80%" placeholder="请输入客人姓名"></el-input>
          </el-form-item>
          <!-- 预定时需要发短信选择 -->
          <el-form-item label="是否发送短信" v-if="formStatus==1">
            <el-radio v-model="formData.customInfo.auto_send_msg" label="2">否</el-radio>
            <el-radio v-model="formData.customInfo.auto_send_msg" label="1">是</el-radio>
          </el-form-item>
        </div>

        <!-- 联合订位人 -->
        <div v-if="formData.secondSales.second_sales_status_arr.indexOf(formStatus)>-1">
          <el-form-item label="联合订位人">
            <input-select style="width:80%" :value="formData.secondSales.second_sales_name" placeholder="请输入联合订位人姓名或工号"
              :optionsList="formData.secondSales.second_sales_info_option" @selectInputHandle="inputSecondSealName"
              @selectOptionItem="changeSecondSealName" @selectBlurHandle="selectBlurHandle"></input-select>
          </el-form-item>
          <el-form-item label="联合订位人电话">
            <el-input v-model="formData.secondSales.second_sales_phone" disabled style="width:80%"></el-input>
          </el-form-item>
        </div>

        <!-- 修改低消金额 -->
        <div v-if="formData.lowConsume.low_consume_status_arr.indexOf(formStatus)>-1">
          <el-form-item label="最低消费" class="required required4">
            <el-input v-model="formData.lowConsume.min_csm_amt" placeholder="请输入最低消费" type="number" min="0" style="width:80%">
            </el-input>
            <span class="m-l-1 white">元</span>
          </el-form-item>
        </div>
        ...
        <!-- 备注 -->
        <el-form-item v-if="formData.remarkInfo.remark_status_arr.indexOf(formStatus)>-1" label="备注">
          <el-input type="textarea" :rows="2" placeholder="请输入备注" v-model="formData.remarkInfo.remark"
            style="width:80%">
          </el-input>
        </el-form-item>

        <!-- 转台 -->
        <div class="change-card" v-if="formData.changeCard.change_card_status_arr.indexOf(formStatus)>-1">
          <div class="change-card-info m-b-4">
            <span>{{formData.changeCard.originCardName}}</span>
            <span>转</span>
            <span>{{formData.changeCard.newCardName}}</span>
            <el-button type="primary" @click.stop="secondChangeCard">更改台位</el-button>
          </div>
          <el-form-item label="老卡台低消" class="required required5">
            <el-input v-model="formData.changeCard.originAmt" disabled type="number" style="width:80%"></el-input>
            <span class="m-l-1 white">元</span>
          </el-form-item>
          <el-form-item label="新卡台低消" class="required required5">
            <el-input v-model="formData.changeCard.new_min_csm_amt" min="0" placeholder="请输入新卡台低消" type="number"
        ...
        <!-- 提交按钮 -->
        <div class="form-btn">
          <el-button type="info" @click="onCancelDrawer">{{formStatus==14?'关闭':'取消'}}</el-button>
          <el-button type="primary" v-if="formStatus!=14" @click="onSubmit">确认</el-button>
        </div>
      </el-form>
      <cardInfoTable v-if="showType == 'table'" :formStatus="formStatus" :cardInfo="cardInfo" :show="show" @closeDrawerHandle="closeDrawerHandle"></cardInfoTable>
    </el-drawer>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getOneDayTimeOption() {
        ...
        for (let i = 0; i < 48; i++) {
          resultOption.push({
            ...
            name: dayName + H.toString().padStart(2, 0) + ':' + M.toString().padStart(2, 0)
          })
          M += 30
          if (M >= 60) {
            H++
      ...
      getArrivedTimeOption() {
        ...
        for (let i = 0; i < 48; i++) {
          ...
          if (!(dayName === '次日' && H >= 12)) resultOption.push({
            id: H.toString().padStart(2, 0) + M.toString().padStart(2, 0),
            name: dayName + H.toString().padStart(2, 0) + ':' + M.toString().padStart(2, 0)
          })
          M += 30
      ...
      getMarkOption(){
        const markList = this.$store.state.cardPageInfo.resResultDataObj.tipsList || []
      ...
      */
      ...
      inputSealName(query) {
        ...
        const results = query ? sealInfoArr.filter(el => el.code.toString().includes(query) || el.name.toString().includes(query) || el.namePy.toString().includes(query.toLowerCase())) : sealInfoArr
      ...
      */
      inputCardCountHandle(e) {
        ...
        this.formData.customPay.customer_exp_cnt = e > 100 ? (e.startsWith('100') ? e.slice(0, 3) : e.slice(0, 2)) : e
      },
      ...
      */
      ...
      inputSecondSealName(query) {
        ...
        const secondSealInfoArr = this.$store.state.cardPageInfo.resResultDataObj.orderPersonInfo.filter(item => item.id !== this.formData.sales.sales_emp_id)
        const results = query ? secondSealInfoArr.filter(el => el.code.toString().includes(query) || el.name.toString().includes(query) || el.namePy.toString().includes(query.toLowerCase())) : secondSealInfoArr
      ...
      // 转台获取新卡台的相关信息
      getNewCardInfo() {
        this.formData.changeCard.new_min_csm_amt = this.$store.state.cardPageInfo.newCardInfo.grpMinCsmAmt   // 新卡台最基础低消
        this.formData.changeCard.newCardName = this.$store.state.cardPageInfo.newCardInfo.name   // 新卡台最基础低消
      },
      // 更改台位信息
      secondChangeCard() {
        this.$emit('showOrHideFullPageHandle', { showFullPage: true, showDrawer: false })
      },
          ...
          case 2:  // 空台、预定开台操作
            if(this.cardInfo.bizStatus.toString() === '1') {
              ...
              if(this.cardInfo.showOnlineText) {
                // 线上卡台开台
                this.showConfirmHandle("确定开台", "线上订台" + this.cardInfo.name + "，开台后不能撤台，是否确认开台？", () => {
                  this.reserveToOpen()
                });
              } else {
                // 线下卡台预定开台
              ...
              this.$message.warning('当前卡台状态为非空台和非预定状态，不需要卡台')
            }
            break
          case 7:  // 空台、预定转台操作
      ...
      async getReservedCardInfo() {
        const sealInfoArr = this.$store.state.cardPageInfo.resResultDataObj.orderPersonInfo
        const secondSealInfoArr = this.$store.state.cardPageInfo.resResultDataObj.orderPersonInfo
        try {
          if(Number(this.bookId) == 0) return
          const res = await api_card.reqGetBookDetail({ id: Number(this.bookId) })
          if (res.code === 1) {

            this.formData.sales.sales_name = sealInfoArr.find(el => el.id == res.data.sales_emp_id) && sealInfoArr.find(el => el.id == res.data.sales_emp_id).name  // 订位人名称
            this.formData.sales.sales_emp_id = res.data.sales_emp_id ? res.data.sales_emp_id.toString() : ''  // 订位人id

            this.formData.sales.sales_phone = sealInfoArr.find(el => el.id == res.data.sales_emp_id) && sealInfoArr.find(el => el.id == res.data.sales_emp_id).phoneNum  // 订位人电话
            this.formData.customArriveInfo.exp_arrive_time = this.getOneDayTimeOption().find(el => el.id == res.data.exp_arrive_time) && this.getOneDayTimeOption().find(el => el.id == res.data.exp_arrive_time).name // 预定到达时间 
            this.formData.customArriveInfo.exp_arrive_time_id = res.data.exp_arrive_time || ''  // 预定到达时间id

            this.formData.customPay.exp_min_csm_amt = res.data.exp_min_csm_amt || '' // 预期最低消费
            this.formData.customPay.card_min_csm_amt = this.cardInfo.grpMinCsmAmt || '' // 卡台固定的最低消费(用于预约之后开台展示)
            this.formData.customPay.customer_exp_cnt = res.data.customer_exp_cnt || '' // 预定人数

            this.formData.customInfo.customer_name = res.data.customer_name || ''  // 客人名称
            this.formData.customInfo.customer_phone = res.data.customer_phone || ''  // 客人电话
            this.formData.customInfo.auto_send_msg = '2'  // 是否发送短信

            this.formData.secondSales.second_sales_name = secondSealInfoArr.find(el => el.id == res.data.second_sales_emp_id) && secondSealInfoArr.find(el => el.id == res.data.second_sales_emp_id).name   // 联合订位人名称
            this.formData.secondSales.second_sales_emp_id = res.data.second_sales_emp_id.toString()  // 联合订位人id
            this.formData.secondSales.second_sales_phone = secondSealInfoArr.find(el => el.id == res.data.second_sales_emp_id) && secondSealInfoArr.find(el => el.id == res.data.second_sales_emp_id).phoneNum   // 联合订位人电话
      ...
      // 空台开台提交
      async nullToOpen() {
        if (this.isValidParams('nullOrReserveToOpenParams') !== 'ok') return false
        try {
          const res = await api_card.reqNullToOpenCard(this.nullOrReserveToOpenParams)
      ...
      async reserveToOpen() {
        if (this.isValidParams('nullOrReserveToOpenParams') !== 'ok') return false
        try {
          const res = await api_card.reqReserveToOpenCard(this.nullOrReserveToOpenParams)
          this.formResponseHandle(res, '开台成功！', false)
        } catch {
          console.log('预定操作出错')
        }
      ...
      async reserveOrOpenToChange() {
        ...
        this.reserveOrOpenToChangeParams.new_min_csm_amt = Number(this.reserveOrOpenToChangeParams.new_min_csm_amt)
        try {
          // cardInfo.bizStatus: 8:预定转台  4：开台转台 
          const res = this.cardInfo.bizStatus == 8 ? await api_card.reqReserveToChangeCard(this.reserveOrOpenToChangeParams) : await api_card.reqOpenToChange(this.reserveOrOpenToChangeParams)
      ...
      async updateConsume() {
        ...
        this.updateConsumeParams.min_csm_amt = Number(this.updateConsumeParams.min_csm_amt)
        try {
          const res = await api_card.reqUpdateLowConsume(this.updateConsumeParams)
      ...
      async updateOpenSales() {
        try {
          const res = await api_card.reqUpdateOpenSales(this.updateOpenSalesParams)
      ...
      async updateOpenType() {
        if (this.updateOpenTypeParams.new_min_csm_amt.length === 0) return this.$message.warning('请填写最低消费')
        if (this.updateOpenTypeParams.reason.length === 0) return this.$message.warning('请填写修改原因')
        try {
          this.updateOpenTypeParams.new_min_csm_amt = Number(this.updateOpenTypeParams.new_min_csm_amt)
      ...
      // 验证提交参数是否合法
      isValidParams(key) {
        if (this.formStatus === 1 || this.formStatus === 8) { // 空台预定1或修改预定信息8
          if (this[key].sales_name && !this[key].sales_emp_id) return this.$message.warning('请输入有效订位人')
          ...
          if (this[key].customer_phone.length !== 0 && this[key].customer_phone.length != 11) return this.$message.warning('请输入正确的手机号')

          if (this[key].second_sales_name && !this[key].second_sales_emp_id) return this.$message.warning('请输入有效联合订位人')
        } else if (this.formStatus === 2) { // 空台或预定开台2
          if (this[key].sales_name && !this[key].sales_emp_id) return this.$message.warning('请输入有效订位人')
      ...
      resetForm() {
        ...
        this.formData.customPay.exp_min_csm_amt = this.cardInfo.grpMinCsmAmt || '' // 预期最低消费
        this.formData.customPay.card_min_csm_amt = '' // 预定开台时卡台固定的最低消费(不可修改状态)
        this.formData.customPay.customer_exp_cnt = this.$store.state.orderInfo.currentCardInfo.capacity // 预定人数
      ...
      cardInfoChange(value) {
        ...
        switch (this.formStatus) {
          ...
          case 11: // 修改订位人、联合订位人
            ...
            const sealInfoArr = this.$store.state.cardPageInfo.resResultDataObj.orderPersonInfo
            const secondSealInfoArr = this.$store.state.cardPageInfo.resResultDataObj.orderPersonInfo
            this.formData.sales.sales_name = sealInfoArr.find(el => el.id == salesEmpId) && sealInfoArr.find(el => el.id == salesEmpId).name  // 订位人名称
            this.formData.sales.sales_emp_id = salesEmpId.toString()  // 订位人id
            this.formData.sales.sales_phone = sealInfoArr.find(el => el.id == salesEmpId) && sealInfoArr.find(el => el.id == salesEmpId).phoneNum  // 订位人电话
            this.formData.secondSales.second_sales_name = secondSealInfoArr.find(el => el.id == secondSalesEmpId) && secondSealInfoArr.find(el => el.id == secondSalesEmpId).name   // 联合订位人名称
            this.formData.secondSales.second_sales_emp_id = secondSalesEmpId.toString()  // 联合订位人id
            this.formData.secondSales.second_sales_phone = secondSealInfoArr.find(el => el.id == secondSalesEmpId) && secondSealInfoArr.find(el => el.id == secondSalesEmpId).phoneNum   // 联合订位人电话
            break
          case 13: // 修改开台类型
            this.formData.lowConsume.min_csm_amt = newVal.assignMinCsmAmt
    ...
    computed: {
      ...
      title() {
        return cardOptions.find(el => el.id === this.formStatus) && cardOptions.find(el => el.id === this.formStatus).name
      },
      ...
      */
      nullOrUpdateToReserveParams() {
        ...
        return {
          ...
          exp_min_csm_amt: formData.customPay.exp_min_csm_amt === 0 ? 0 : formData.customPay.exp_min_csm_amt * 1,     // int64  预期最低消费
      ...
      nullOrReserveToOpenParams() {
        ...
        return {
          ...
          min_csm_amt: this.showMinAmtOpenTypeArr.indexOf(formData.openType.open_type) > -1 ? (formData.customPay.exp_min_csm_amt === 0 ? 0 : formData.customPay.exp_min_csm_amt * 1) : 0, // int 最低消费
          remark: formData.remarkInfo.remark,     // string  备注信息
      ...
      reserveOrOpenToChangeParams() {
        return {
          // cardInfo.bizStatus: 8:预定转台  4：开台转台 
          id: this.cardInfo.bizStatus == 8 ? Number(this.bookId) : Number(this.cardId),      // int 待操作卡台Id
      ...
      */
      updateConsumeParams() {
        ...
        return {
          ...
          min_csm_amt: this.formData.lowConsume.min_csm_amt === 0 ? '0' : this.formData.lowConsume.min_csm_amt.toString()  // int  抵消金额(元) 此处暂用string，用于判断是否为空
        }
      },
      ...
      updateOpenTypeParams() {
        ...
        return {
          id: Number(this.cardId),    //    int64  待操作卡台Id
          open_type: formData.openType.open_type,  //int   开台类型 1 正常消费,2 内招, 3 外招, 4 气氛, 5 全转, 6 半转
          new_min_csm_amt: this.showMinAmtOpenTypeArr.indexOf(formData.openType.open_type) > -1 ? (formData.lowConsume.min_csm_amt === 0 ? '0' : formData.lowConsume.min_csm_amt.toString()) : '0', // int  抵消金额(元)  此处暂用string，用于判断是否为空
    ...
    watch: {
      showDrawer(newVal) {
        ...
        if(newVal && this.formStatus == 14){
          const saleInfo = common_book.getOrderPersonInfo(this.cardInfo.salesEmpId) || {}
          const secondSaleInfo = common_book.getOrderPersonInfo(this.cardInfo.secondSalesEmpId) || {}
