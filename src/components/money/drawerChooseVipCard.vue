  ...
  <div>
    ...
    >
      <div class="session choose-vip-card">
        <div class="form">
          <template v-if="type == 1">
            <div class="row" layout="row" layout-align="start center">
              ...
              <div class="value" layout="row" layout-align="start center">
                <input type="text" v-model="validateVal" placeholder="请输入付款码序列号" />
          ...
          <template v-else>
            <div class="row" layout="row" layout-align="start center">
              ...
              <div class="value" layout="row" layout-align="start center">
                <input type="text" v-model="phoneNumVal" placeholder="请输入手机号" />
                <div class="btn" :class="{disabled: count<60}" @click="sendPhoneMessage">{{btnText}}</div>
              </div>
            </div>
            <div class="row" layout="row" layout-align="start center">
              <div class="label">
              ...
              <div class="value" layout="row" layout-align="start center">
                <input type="text" v-model="validateVal" placeholder="请输入验证码" />
                <div class="btn" @click="getTableData">搜索</div>
              </div>
            </div>
          </template>
        ...
        <p class="tips red">提示：勾选会员卡后，方可填写使用金额</p>
...
export default {
  ...
  methods: {
    ...
    async getTableData() {
      const result = this.type == 1 ? await this.validateValidateInfo() : await this.validatePhoneInfo();
      if (result) {
        const params = {
    ...
    async onSubmit() {
      const params = {
        ...
        mb_card_ids: this.tableData.filter(item => item.checked).map(item => item.id * 1), // []int64    会员卡Id列表
        amts: this.tableData.filter(item => item.checked).map(item => item.chooseAmt.toString()), //       []string   使用金额
        ...this.payId && { pay_id: this.payId * 1 }
      };
      try {
        const api = this.payId ? 'reqAddVipCardFormPhoneNumInPayAfter' : 'reqAddVipCardFormPhoneNumInPayBefore'
