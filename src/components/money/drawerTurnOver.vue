...
export default {
  ...
  methods: {
    ...
    async getTableData() {
      try {
        const res = await api_money.reqGetTurnOverList();
        if (res.code == 1) {
    ...
    async exportExcel() {
      try {
        const res = await api_money.reqExportTurnOverListReport();
        if (!res.msg) {
    ...
    onCancelDrawer() {
      this.$emit("showOrHideTurnOverDetailDrawer");
    },

    resetHandle() {
      // this.dateVal = this.getDate()
      this.getTableData();
    }
  },
  props: {
