    ...
    >
      <div class="content" layout="row" layout-align="center center">
        ...
        <div class="center">
          <div class="center-top" layout="row" layout-align="center center">
            <div class="center-top-left" layout="row" layout-align="start center">
              <p>未收:</p>
          ...
          <div class="vip-com" v-if="payActiveInfo.id==5">
            <div class="form-item">
              <span>会员卡卡号:</span>
              <input ref="vipNumRef" :class="{'active': vipInfo.activeInput == 1}" @focus="vipInfo.activeInput=1" @input="inputHandle" v-model="vipInfo.vipNum" placeholder="请输入会员卡卡号"/>
              <i class="el-icon-circle-close" :style="{visibility: vipInfo.vipNum ? 'visible' : 'hidden'}" style="color:rgba(255,255,255,.3);transform:translateX(-30px);cursor:pointer;" @click="vipInfo.vipNum=''"></i>
            </div>
            <div class="form-item">
              <span>卡储值金额:</span>
              <input :class="{'active': vipInfo.activeInput == 2}" @focus="vipInfo.activeInput=2" @input="inputHandle" v-model="vipInfo.payAmt" placeholder="请输入卡储值金额"/>
              <i class="el-icon-circle-close" :style="{visibility: vipInfo.payAmt ? 'visible' : 'hidden'}" style="color:rgba(255,255,255,.3);transform:translateX(-30px);cursor:pointer;" @click="vipInfo.payAmt=''"></i>
            </div>
            <div class="form-item">
              <span>卡折扣金额:</span>
              <input :class="{'active': vipInfo.activeInput == 3}" @focus="vipInfo.activeInput=3" @input="inputHandle" v-model="vipInfo.paySubAmt" placeholder="请输入卡折扣金额"/>
              <i class="el-icon-circle-close" :style="{visibility: vipInfo.paySubAmt ? 'visible' : 'hidden'}" style="color:rgba(255,255,255,.3);transform:translateX(-30px);cursor:pointer;" @click="vipInfo.paySubAmt=''"></i>
            </div>
          </div>

          <!-- 会员卡 -->
          <div class="vip-pay vip-com" v-else-if="payActiveInfo.id==500">
            <div class="form-item" layout="row" layout-align="space-around center" style="padding: 0 15%">
              <span style="color:#FF2F64">会员卡刷卡</span>
              <span style="width:130px;color:#4b89ff;cursor:pointer" @click="showOrHideChooseVipCardDrawer(1)">服务码选择会员卡</span>
              <span style="width:130px;color:#4b89ff;cursor:pointer" @click="showOrHideChooseVipCardDrawer(2)">手机号选择会员卡</span>
            </div>
            <div class="form-item">
              <span>会员卡卡号:</span>
              <span class="value">{{vipPayInfo.cardNo}}{{vipPayInfo.name ? '(' + vipPayInfo.name + ')' : ''}}</span>
            </div>
            <div class="form-item">
              <span>当前余额:</span>
              <span class="value">￥{{(vipPayInfo.totalAmt * 1).toFixed(2)}}</span>
            </div>
            <div class="form-item">
              <span>支付金额:</span>
              <span class="count value" layout="row" layout-align="start center" style="position:relative">
                <input class="count m-l-1" ref="moneyCountRef" v-model="count" style="width:calc(100% - 30px);height:100%;border:none;transform:translateY(-8px)" placeholder="请输入金额"/>
                <i class="el-icon-circle-close" :style="{visibility: count ? 'visible' : 'hidden'}" style="position:absolute;right:-26px;top:10px;color:rgba(255,255,255,.3);transform:translateX(-30px);cursor:pointer;" @click="count=''"></i>
              </span>
            </div>
          </div>
          ...
          <div v-else-if="payActiveInfo.id==9999" class="center-center m-t-3">
            <div class="form-item" layout="row" layout-align="start center" style="padding: 0 15%; flex-wrap:wrap;">
              <div 
                ...
                :key="item.id">
                <div class="title" style="text-align:center"> {{item.n.split('|')[0]}} </div>
                <div class="time m-t-1" style="text-align:center"> {{item.n.split('|')[1]}} </div>
              </div>
            </div>
          </div>
          ...
          <div v-else-if="payActiveInfo.id==6" class="center-center m-t-3">
            ...
            <p class="m-t-3" layout="row" layout-align="start center">
              ...
              <span class="count" layout="row" style="position:relative">
                ￥<input class="count m-l-1" ref="moneyCountRef" v-model="count" style="width:140px;height:100%;border:none;transform:translateY(-2px)" placeholder="请输入金额"/>
                <i class="el-icon-circle-close" :style="{visibility: count ? 'visible' : 'hidden'}" style="position:absolute;right:-26px;top:10px;color:rgba(255,255,255,.3);transform:translateX(-30px);cursor:pointer;" @click="count=''"></i>
              </span>
            </p>
          </div>
          ...
          <div v-else class="center-center m-t-8">
            <p layout="row" layout-align="start center">
              <span>{{payActiveInfo.name}}金额:</span>
              <span class="count" layout="row" style="position:relative">
                ￥<input class="count m-l-1" ref="moneyCountRef" v-model="count" style="width:calc(100% - 30px);height:100%;border:none;transform:translateY(-2px)" placeholder="请输入金额"/>
                <i class="el-icon-circle-close" :style="{visibility: count ? 'visible' : 'hidden'}" style="position:absolute;right:-26px;top:10px;color:rgba(255,255,255,.3);transform:translateX(-30px);cursor:pointer;" @click="count=''"></i>
              </span>
            </p>
            <p
          ...
          <div v-if="payActiveInfo.id!=9999" class="center-bottom" layout="row" layout-align="center center">
            <ul>
            ...
            <ul>
              ...
              >
                <div class="pay-detail">
                  <!-- 会员卡 -->
                  <p v-if="item.pid==500">{{item.cn}}{{item.l==1 ? '('+ item.lt +')' : ''}}{{item.n ? '(' + item.n + ':' : '('}}{{item.c + ')'}}:</p>
                  <!-- 非会员卡 -->
                  <p v-else>{{item.cn}}{{item.l==1 ? '('+ item.lt +')' : ''}}:</p>

                  <!-- 会员卡落单/会员卡 -->
                  <div v-if="item.pid==5 || item.pid==500">
                    <p class="amt" layout="row" layout-align="start center" style="font-size:13px"><span style="width:76px;">卡金额:</span>￥{{(item.pa * 1).toFixed(2)}}</p>
                    <p class="amt" layout="row" layout-align="start center" style="font-size:13px"><span style="width:76px;">卡储值金额:</span>￥{{(item.pv * 1).toFixed(2)}}</p>
                    <p class="amt" layout="row" layout-align="start center" style="font-size:13px"><span style="width:76px;">卡折扣金额:</span>￥{{(item.pf * 1).toFixed(2)}}</p>
                  </div>
                  <p v-else class="amt">￥{{item.chooseAmt}}</p>
                </div>
                <div class="close">
                  <i class="el-icon-error" @click.stop="delPayList(item.id)"></i>
                </div>
              </li>
            </ul>
          </div>
      ...
      <div class="form-btn" layout="row" layout-align="start center">
        ...
        <div class="btn-right" layout="row" layout-align="end center">
          <el-button class="m-r-6" type="primary" @click="onSubmit">收款</el-button>
        </div>
      </div>

      <authComponent
        ref="authComponent"
        :amt="this.count - this.allAmt > 0? this.allAmt.toString(): this.count.toString()"
...
export default {
  ...
  methods: {
    ...
    inputHandle(){
      const splitStr = 'tab';
      const numList = [this.vipInfo.vipNum,  this.vipInfo.payAmt, this.vipInfo.paySubAmt]
    ...
    async init() {
      ...
      let allPayList = this.$store.state.cardPageInfo.resResultDataObj["payList"].filter(item => item.status == 1 && (item.id < 200 || item.id == 500))
      // 滞留金信息
      let bookPayInfo = {}
      ...
      // 门店设置的渠道
      const effectPayList = this.$store.state.cardPageInfo.resResultDataObj["payListFilter"].filter(item => item.status == 1)
      ...
      if(effectPayList.length > 0) {
        // 门店有配置
        payList = canConfigPayList.filter(item => effectPayList.map(items => items.id).includes(item.id))
      } else {
        payList = [...canConfigPayList]
      }
      // 线下支付订单
      this.payList = bookPayInfo.id ? [{...bookPayInfo}, ...payList, ...vipPayList].sort((a, b) => a.dsp - b.dsp) : [...payList, ...vipPayList].sort((a, b) => a.dsp - b.dsp);
    ...
    async addBookItemToShoppingCartHandle(itemInfo){
      if(this.notPayAmt * 1 < itemInfo.a * 1) return this.$message.warning('未支付金额'+this.notPayAmt+'元，不能使用大于该金额的滞留金，可添加其他待结商品或扣减其他结账渠道')
      const params = {
        id: this.$store.state.orderInfo.currentCardInfo.seatId * 1, //  int64   卡台Id
    ...
    async selfAuth() {
      const params = {
        ...
        pay_amt: this.count - this.allAmt + this.chooseAmt * 1 > 0
    ...
    // 加入已选择好的支付渠道列表
    async addChooseList() {
      if (!this.count && this.payActiveInfo.id!=5) return this.$message.warning("请输入金额");
      ...
      try {
        ...
        if(this.payActiveInfo.id == 5) {
          params = {
            ...params,
            pay_amt: (this.vipInfo.payAmt * 1 + this.vipInfo.paySubAmt * 1).toFixed(2),
          ...
          if(params.card_no === '') return this.$message.warning('请输入会员卡号');
          if(params.card_value_amt === '') return this.$message.warning('请输入卡金额');

        }

        // 挂账
        if(this.payActiveInfo.id == 6) {
          if(this.GZInfo.selectInfo.selectVal == '') return this.$message.warning('请选择挂账账户')
          params = {
            ...params,
            biz_id: this.GZInfo.originOption.find(item => item.name == this.GZInfo.selectInfo.selectVal).id * 1
          }
        }
    ...
    readCard() {
      ...
      try {
        setTimeout(() => {
          ...
          if (!cardNo) {
            if (this.show && this.payActiveInfo.id == 500 && !this.showChooseVipCardDrawer) this.readCard();
          } else {
    ...
    async addVipInfo() {
      ...
      try {
        const res = await api_money.reqAddVipCardFormPhoneNumInPayBefore(params);
        if (res.code == 1) {
          this.$message.success("加入成功");
          this.getChoosePayList();
        } else {
    ...
    payActiveInfo: {
      handler(newVal){
        ...
        if(newVal.id == 6) {
          ...
          this.GZInfo.originOption = this.$store.state.cardPageInfo.resResultDataObj.accountOfGZ.filter(item => item.status == 1)
        }
      },
      deep: true
    }
