  ...
  <div>
    <div class="order-info" layout="row" layout-align="center start">
      ...
      <div class="order-info-left">
        ...
        <div class="ul" :class="{'show-change-btn':payTabList.length==3}">
          <div class="li" v-for="(item,i) in payedList" :key="i">
            <div class="pay-detail">
              <p v-if="item.choosedPayInfo.id == 500">{{item.cn}}{{item.n ? '(' + item.n + ':' : '('}}{{item.c + ')'}}:</p>
              <p v-else>{{item.cn}}{{item.n ? '( ' + item.n + ')' : ''}}:</p>
              <!-- 会员卡落单 -->
              <div v-if="item.choosedPayInfo.id == 5 || item.choosedPayInfo.id == 500">
                <p class="amt" style="font-size:12px" layout="row" layout-align="start center"><span style="width:70px;">总金额:</span>￥{{(item.pa * 1).toFixed(2)}}</p>
                <p class="amt" style="font-size:12px" layout="row" layout-align="start center"><span style="width:70px;">卡储值金额:</span>￥{{(item.pv * 1).toFixed(2)}}</p>
                <p class="amt" style="font-size:12px" layout="row" layout-align="start center"><span style="width:70px;">卡折扣金额:</span>￥{{(item.pf * 1).toFixed(2)}}</p>
              </div>
        ...
        <div
          class="change-type"
          v-if="showChangeTypeBtn && !(payedOrderList[0] ? payedOrderList[0].isOnlinePay : false)"
                    ...
                    <div class="td" style="padding:0">
                      <div v-for="(items,index) in item.resultNotPayData" :key="index">
                        <div class="td-tr" layout="row" layout-align="start center">
                          ...
                          >{{(items.pp*1).toFixed(2)}}</div>
                          <div
                            class="td-td"
                            :class="{'opacity':items.back}"
                          >{{items.at==2 || items.at==3 ? '0.00' : items.pp==0? (items.pa*1).toFixed(2) : (((items.pp * 1 == 0) ? (items.pa * 1) : (items.pp * items.pc)).toFixed(2))}}</div>
                          <div
                            class="td-td"
                            :class="{'opacity':items.back}"
                          ...
                          <div
                            class="td-td"
                            :class="{'opacity':items.back}"
                          >{{items.authInfo ? items.authInfo.name : '自助'}}</div>
                ...
                <div class="another-order" v-else>
                  <div class="detail tr">
                    <div class="td" layout="row" layout-align="start center">
                      ...
                      <span class="gray-tips" v-if="item.im==2 && item.isOnlinePay">
                        <span>出</span>
                        <img src="@/assets/money-img/notOut.png" />
                      </span>
                    </div>
                    <div class="td" :class="{'opacity':item.back}">{{item.productInfo && item.productInfo.name}}</div>
                    ...
                    >{{item.at==2 || (item.at==3 && !(item.io==1 || item.oid)) ? '0.00' : (((item.pp * 1 == 0) ? (item.pa * 1) : (item.pp * item.pc)).toFixed(2))}}</div>
                    <div
                      class="td"
                      :class="{'opacity':item.back}"
                    ...
                    <div class="td">
                      <img
                        ...
                        v-if="item.is==1&&turnOverCount==$store.state.orderInfo.currentCardInfo.turnoverCnt"
...
export default {
  ...
  methods: {
    ...
    showOrHideDrawer() {
      if(!this.showDrawer) {
        this.$confirm('如果继续操作，本订单已支付渠道将进入滞留金列表，本订单需重新支付。请确认是否继续操作？', '提示', {
