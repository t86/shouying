    ...
    >
      <div class="session add-update-vip-type">
        ...
        <template v-if="status==1">
          <div class="top-info">
            <p>
              <span class="bold m-r-2">卡类型信息</span>
              <span>不同的卡类型用处不同，例如：普通会员卡用于所有消费；香槟卡只适用于香槟消费</span>
            </p>
            <div class="form">
              <div class="row" layout="row" layout-align="start center">
                ...
                <el-input
                  ...
                  size="small"
                  placeholder="请输入会员卡类型名称(1-10字)"
                  style="width:284px;"
                  @blur="updateVipNameHandle"
                ></el-input>
              </div>
              <div class="row" layout="row" layout-align="start center">
                <div class="label">设为默认类型:</div>
                <el-switch v-model="form.isDefault" :disabled="canDisabledDefault || (editInfo.id && form.isDefault)" @change="updateVipDefaultHandle" style="width:56px;height:32px"></el-switch>
              </div>
      ...
      <div class="form-btn" layout="row" layout-align="center center">
        ...
        <el-button v-if="!(editInfo.id && subStatus == 1)" type="primary" @click="onSubmit">确定</el-button>
      </div>
    </el-drawer>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async resetData(){
      this.status = 1
      this.subStatus = 1
      this.imgBaseUrl = imgBaseUrl = await this.getImgBaseUrl()
      this.form = {
        vipName: "",
        isDefault: this.canDisabledDefault ? true : false
      }
