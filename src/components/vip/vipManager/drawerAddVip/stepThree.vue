    ...
    <div class="form">
      <div class="row" layout="row" layout-align="start center">
        ...
        <div class="value" layout="row" layout-align="start center">
          <el-input v-model="form.vipNumVal" :maxlength="10" size="small" style="width:284px" placeholder="请输入会员卡号"></el-input>
          <div v-if="loading" class="loading" layout="row" layout-align="start center">
      ...
      <div class="row" layout="row" layout-align="start center">
        ...
        <div class="value" layout="row" layout-align="start center">
          <el-select v-model="form.vipTypeVal" size="small" style="width:284px" placeholder="请选择会员卡类别">
            <el-option
      ...
      <div class="row" layout="row" layout-align="start center">
        ...
        <div class="value" layout="row" layout-align="start center">
          <el-select v-model="form.vipDeepVal" size="small" style="width:284px" placeholder="请选择会员卡等级">
            <el-option
      ...
      <div class="row" layout="row" layout-align="start center">
        ...
        <div class="value">
          <el-input v-model="form.userName" size="small" style="width:284px" placeholder="请输入姓名"></el-input>
        </div>
      </div>
      <div class="row" layout="row" layout-align="start center">
        <div class="label">
      ...
      <div class="row" layout="row" layout-align="start center">
        ...
        <div class="value">
          <el-radio v-for="item in form.markList" :key="item" v-model="form.markVal" :label="item">{{item}}</el-radio>
        </div>
      </div>
      <div class="row" layout="row" layout-align="start center">
        <div class="label">
        ...
        <div class="value">
          <el-radio v-for="item in form.timeLongList" :key="item.id" v-model="form.timeLongVal" :label="item.id">{{item.n}}</el-radio>
        </div>
      </div>
      <div v-if="stepOneInfo.radioVal != 2" class="row" layout="row" layout-align="start center">
        <div class="label">
          <span>订阅短信类型:</span>
        </div>
        <div class="value">
          <el-checkbox v-model="item.checked" v-for="item in form.messageList" :key="item.id">{{item.n}}</el-checkbox>
        </div>
      </div>
      <div class="row" layout="row" layout-align="start center">
        <div class="label">
...
export default {
  ...
  methods: {
    ...
    async getParams(){
      try {
        ...
        if (res.code == 1) {
          ...
          const defaultVipTypeInfo = this.form.vipTypeOption.sort((a, b) => a - b)[0]
          this.form.vipTypeVal = defaultVipTypeInfo.id
          this.form.vipDeepValOption = defaultVipTypeInfo.ls || []
          const defaultVipDeepInfo = this.form.vipDeepValOption.sort((a, b) => a - b)[0]
    ...
    remoteMethod(query) {
      if (query !== "") {
        ...
        this.form.openCardPersonOptions = JSON.parse(JSON.stringify(this.openCardPersonOptions)).filter(item => item.code.includes(query))
  ...
  watch: {
    'form.vipTypeVal'(newVal){
      const defaultVipTypeInfo = this.form.vipTypeOption.find(item => item.id == newVal) || {}
      this.form.vipDeepValOption = defaultVipTypeInfo.ls || []
      const defaultVipDeepInfo = this.form.vipDeepValOption.sort((a, b) => a - b)[0]
      this.form.vipDeepVal = defaultVipDeepInfo.id
    },
