    ...
    getMenuInfo() {
      ...
      if(true) {
        if(this.$store.state.userInfo.authStatusArr.includes(1)) {
          // 服务员
          // 查看是否为不限可点
          if (notLimitStationId.map(item => item.stationId * 1).includes(authStationId * 1)) {
            // 当前岗位可点商品没有限制
            // 此时此刻不需要进行岗位限制的筛选
            // 只需要获取所有的商品二级分类即可
            // 此处为了配合接下来的流程中区域筛选，将数据处理成与有限制的可点商品二级分类一致的
            ...
            FWYSecondCategory = secondCategory.filter(
              el => el.stationId == authStationId && el.authType == 1  // authType:1  服务员可点
            );
          }
        }
      ...
      function getSealConfigPrdId(that, type=2){
        const limitRoleFreeLimitIdList = that.$store.state.cardPageInfo.resResultDataObj["roleLimitConfig"].filter(item => item.status == 1 && item.station_id == authStationId && item.auth_type == type).map(item => item.free_limit_id * 1)
        
        // 商品id
        const prdListId = that.$store.state.cardPageInfo.resResultDataObj["limitPrdDetail"].filter(item => item.status == 1 && limitRoleFreeLimitIdList.includes(item.free_limit_id * 1)).map(item => item.prd_id * 1)
      ...
      if (this.$route.name == 'moneyCard' && this.$store.state.userInfo.authStatus == 4) {
        // 估清
        currentAreaAllProduct = this.$store.state.cardPageInfo.resResultDataObj["areaProduct"].filter(el => el.status == 1);  
      } else {
        // 服务员  或  营销  或  花篮  或  优惠  或  优惠2
        currentAreaAllProduct = this.$store.state.cardPageInfo.resResultDataObj["areaProduct"].filter(el =>
          el.status == 1 &&
      ...
      // 所有二级分类下所有单品和套餐
      let stationAllProduct = this.$store.state.cardPageInfo.resResultDataObj["goodsAroundInfo"].filter(el => el.status == 1);
      ...
      if(this.$store.state.userInfo.authStatusArr.includes(1)) {
        ...
        currentAreaAllProduct.forEach(el => {
          ...
          const find = stationAllProduct.find(item => item.id == el.prd_id && item.useType != 2)
          if (find) FWYAreaPrdList.push({
            ...find,
            canOrderMeal: true
          })
        })
        ...
        FWYSecondCategory.forEach(el => {
          ...
          const findList = stationAllProduct.filter(item => el.twoCateId == item.twoCateId && item.useType != 2 && item.prdType != 3 && item.prdType != 4 && item.prdType != 13 && item.prdType != 14)
        ...
        FWYAreaPrdList.forEach(el => {
          const find = FWYSecondCategoryPrdList.find(item => item.id == el.id)
      ...
      if(this.$store.state.userInfo.authStatusArr.includes(2)) {
        ...
        currentAreaAllProduct.forEach(el => {
          ...
          if (find) YXAreaPrdList.push({
            ...find,
            canSeal: true
          })
        })

        const prdList = stationAllProduct.filter(item => prdListId.includes(item.id * 1))
      ...
      if(this.$store.state.userInfo.authStatusArr.includes(5)) {
        ...
        currentAreaAllProduct.forEach(el => {
          ...
          const find = stationAllProduct.find(item => item.id == el.prd_id && (item.prdType != 2 && item.prdType != 3 && item.prdType != 4 && item.prdType !=5))
          if (find) YH2AreaPrdList.push({
            ...find,
            canSealYH2: true
          })
        })

        const prdList = stationAllProduct.filter(item => YH2PrdListId.includes(item.id * 1))
      ...
      if(this.$store.state.userInfo.authStatusArr.includes(3)) { 
        ...
        const authFlowerPrdList = this.$store.state.cardPageInfo.resResultDataObj["authFlowerPrdList"].filter(item => item.status == 1 && item.station_id == this.$store.state.userInfo.station_id)
        ...
        stationAllProduct.forEach(el => {
          ...
          if (findProduct) HLCanHLPrdList.push({
            ...el,
            canHL: true
          });
        });
      ...
      if(true) {
        ...
        const tempResultProductArrList = [...FWYAllProductList, ...YXAllProductList].filter((item, index, arr) => arr.findIndex(items => item.id == items.id) == index)
        ...
        const tempResultProductArrList2 = [...tempResultProductArrList, ...YH2AllProductList].filter((item, index, arr) => arr.findIndex(items => item.id == items.id) == index)
        ...
        const tempResultProductArrList3 = [...tempResultProductArrList2, ...HLAllProductList].filter((item, index, arr) => arr.findIndex(items => item.id == items.id) == index)
        mergePrdList3.forEach(el => {
          const findMergePrd = tempResultProductArrList2.find(item => item.id == el.id)
      ...
      if(this.$store.state.orderInfo.currentCardInfo.bizType == 3) {
        resultProductArr = resultProductArr.filter(item => (!item.canSeal && !item.canSealYH2) || item.canHL || item.canOrderMeal)
      ...
      secondCategoryAll.forEach(el => {
        ...
        if (secondCategoryInfo) secondCategoryInfoArr.push({
          ...el,
          twoCateId: el.id
        });
      });
      ...
      if(this.$route.name == 'orderMealList') {  // 点单系统
